      SUBROUTINE IO_VECTOR_INT(COMAND,DEVICE,IUNIT,VECTOR_M,VECTOR,VMAX,
     &  FILEFORMAT,INOUTTYPE,ERROR,*)

C#### Subroutine: IO_VECTOR_INT
C###  Description:
C###    IO_VECTOR_INT reads and writes a single (integer) vector.
...
C     VECTOR_M = Number of rows in the vector
      ...
      INCLUDE 'mach00.inc'
!     Parameter List
      INTEGER IUNIT,VECTOR_M,VECTOR(*),VMAX
...
!     Local Variables
     ...
     '  NUMVALUES,NUMVALS(1),row,SPARSITY(1),TIMECODE(1)
      ...
      CALL ENTERS('IO_VECTOR_INT',*9999)
...
C         Read the sparsity type
     ...
     '      INOUTTYPE(1:9).EQ.'ALL_INOUT') THEN
C           Read the values
            CALL BINREADFILE(IUNIT,INTTYPE,NUMVALUES,INTDATA,
...
C           Write out vector values
     ...
     '        INOUTTYPE(1:9).EQ.'ALL_INOUT') THEN
              ...
              WRITE(IUNIT,'(5(1X,I7),:/(5(1X,I7)))') (VECTOR(row),
     '          row=1,VECTOR_M)
            ENDIF               !array/all_inout
...
c            WRITE(OP_STRING,'(1X,I7,:/(1X,I7))') (VECTOR(row),
c     &        row=1,VECTOR_M)
c           CALL WRITES(IUNIT,OP_STRING,ERROR,*9999)

            DO row=1,VECTOR_M
              WRITE(OP_STRING,'(2(1X,I7))') row,VECTOR(row)
              CALL WRITES(IUNIT,OP_STRING,ERROR,*9999)
            ENDDO !row
...
C         Write the values stored
          IF(INOUTTYPE(1:6).EQ.'ARRAYS'.OR.
     '      INOUTTYPE(1:9).EQ.'ALL_INOUT') THEN
            CALL BINWRITEFILE(IUNIT,INTTYPE,VECTOR_M,INTDATA,REAL4DATA,
     '        VECTOR,CHARDATA,LOGDATA,SINTDATA,ERROR,*9999)
      ...
      CALL EXITS('IO_VECTOR_INT')
      RETURN
 9999 CALL ERRORS('IO_VECTOR_INT',ERROR)
      CALL EXITS('IO_VECTOR_INT')
