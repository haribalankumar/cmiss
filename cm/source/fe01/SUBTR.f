      SUBROUTINE SUBTR(STR1,STR2,ERROR,*)

C#### Subroutine: SUBTR
C###  Description:
C###    SUBTR replaces STR1 with STR1-STR2.

      IMPLICIT NONE
      INCLUDE 'b01.cmn'
      INCLUDE 'cbdi02.cmn'
!     Parameter List
      CHARACTER STR1*(*),STR2*(*),ERROR*(*)
!     Local Variables
      INTEGER i,IEND,j,nolv,norl,NTILV1(10),NTILV2(10),NTLV1,NTLV2,
     '  NTRL1,NTRL2
      REAL*8 RL1(100),RL2(100),SUM
      CHARACTER CHARTEMP*20
      LOGICAL SCALR1,SCALR2

      CALL ENTERS('SUBTR',*9999)
      IF(STR1(1:1).EQ.'[') THEN
        SCALR1=.FALSE.
...
CC$        call mp_unsetlock()
      ...
      IF(SCALR1.AND.SCALR2) THEN
        SUM=RL1(1)-RL2(1)
C        STR1=CFROMR(SUM,'E15.8')
        WRITE(STR1,'(E15.8)') SUM
      ELSE IF(.NOT.SCALR1) THEN
        STR1(1:1)='['
        IEND=1
        DO i=1,NTILV1(1)
          IF(NTLV1.GT.1) THEN
          ...
          DO j=1,NTILV1(2)
            ...
            ELSE
              SUM=RL1((i-1)*NTILV1(2)+j)-RL2((i-1)*NTILV1(2)+j)
            ENDIF
      ...
      ELSE IF(.NOT.SCALR2) THEN
        ...
        DO i=1,NTILV2(1)
          ...
          DO j=1,NTILV2(2)
            ...
            ELSE
              SUM=RL1((i-1)*NTILV2(2)+j)-RL2((i-1)*NTILV2(2)+j)
            ENDIF
            WRITE(CHARTEMP,'(E15.8)') SUM
            STR1=STR1(1:IEND)//CHARTEMP//','
C news MPN 28Jul97: CHARTEMP is now 20 chars long (not 15)
