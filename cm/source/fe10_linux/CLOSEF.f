...
C access stream_lf file.
            ...
            IF(IOSTAT.LE.0) THEN
              DO I=1,IREC-1
                ...
                IF(IOSTAT.LE.0) THEN
                  ...
                  WRITE(UNIT=99,FMT='(A)',IOSTAT=IOSTAT) LINE(:IEND)
                  IF(DOP) THEN
     ...
     '        ACCESS='SEQUENTIAL',FORM='FORMATTED',IOSTAT=IOSTAT)
            ...
            IF(iostat.GT.0) GOTO 9998
          ENDIF !iold
C         With Intel compilers on Linux and with xlf on AIX, we will get
C         a close error when the fortran library tries to unlink the
C         scratch file (which we have already unlinked).
C         The IOSTAT from Intel fortran compiler is 175.
C         g77 doesn't report any error.
C         I can't think of any significant close errors as we are not
C         wanting to keep the file.  Therefore ignore close errors.
          CLOSE(UNIT=IUNIT,IOSTAT=IOSTAT)
C          IF(IOSTAT.NE.0) GOTO 9998
