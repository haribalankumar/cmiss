      SUBROUTINE ASSEMBLE6(IBT,IDO,INP,ISC_GD,ISC_GK,ISC_GM,ISR_GD,
     '  ISR_GK,ISR_GM,LGE,NBH,NBJ,NEELEM,NHE,NKJE,NORD,NPF,NPNE,
     '  nr,NVHE,NVJE,NW,nx,NYNE,NYNP,NYNR,CE,CG,CGE,CP,ED,EM,ER,ES,
     '  GD,GK,GM,GR,PG,RG,SE,WG,XA,XE,XG,XP,YG,ZE,ZG,DYNAM1,DYNAM2,
     '  UPDATE_MATRIX,ERROR,*)

C#### Subroutine: ASSEMBLE6
C###  Description:
C###    ASSEMBLE6 assembles the global unreduced matrices GK,
C###    GM, etc. for Modal analysis problems.
...
!     Parameter List
     ...
     '  NEELEM(0:NE_R_M,0:NRM),NHE(NEM),NKJE(NKM,NNM,NJM,NEM),
     '  NORD(5,NE_R_M),NPF(9,NFM),NPNE(NNM,NBFM,NEM),
     '  nr,NVHE(NNM,NBFM,NHM,NEM),NVJE(NNM,NBFM,NJM,NEM),
...
!     Local Variables
      INTEGER ne,NHST(2),nhs1,nhs2,noelem,
     '  no_nynr1,NO_TIMES,ny1,ny2,nz
      REAL AVETIME,ELAPSED_TIME,TIME_START1(1),TIME_START2(1),
     '  TIME_START3(1),TIME_STOP(1)

      CALL ENTERS('ASSEMBLE6',*9999)
...
C***  Initialise matrices for this region
     ...
     '    KTYP24,GK,ERROR,*9999)
        IF(DYNAM1) THEN
C cpb 10/4/96 At the moment the sparsity for the GD and GM matrices
...
C*** Assemble element matrices into global matrices
     ...
     '    //'D11.4,'' s'')') ELAPSED_TIME
      ...
      CALL EXITS('ASSEMBLE6')
      RETURN
 9999 CALL ERRORS('ASSEMBLE6',ERROR)
      CALL EXITS('ASSEMBLE6')
      RETURN 1
      END
