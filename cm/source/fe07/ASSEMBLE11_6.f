      SUBROUTINE ASSEMBLE11_6(ISC_GKK,ISR_GKK,NEELEM,NENQ,
     '  NP_INTERFACE,NPLIST,NPNODE,NQGP,NQGP_PIVOT,NQLIST,NQLIST2,
     '  NQLIST3,NQNP,NQNP_LOCAL,NQNP_LOCAL_PIV,NQS,NQXI,nr,nr2,nr3,
     '  SYSSIZE,NWQ,nx,nx2,nx3,NXQ,CQ,D_MATRIX,DNUDXQ,DXDXIQ,DXDXIQ2,
     '  GCHQ,GK,GK2,GKK,GQ,GQ2,GQ_D_MATRIX,GKGK2,GQGQ2,GUQ,NQGW,PROPQ,
     '  YQ,FIXQ,SOLVEEIGHTPROBLEM,ERROR,*)

C#### Subroutine: ASSEMBLE11_6
C###  Description:
C###    <HTML> <PRE>
C###    ASSEMBLE11_6 creates matrices for coupled BEM-FD
...
!     Parameter list
     ...
     '  PROPQ(3,3,4,2,NQM),YQ(NYQM,NIQM,NAM,NXM)
      CHARACTER ERROR*(*)
      LOGICAL FIXQ(NYQM,NIYFIXM,NXM),SOLVEEIGHTPROBLEM
!     Local variables
      INTEGER BCINNBND,DUMMY_LIST(0:1),
     '  IDUMMY,ncol,NCOUPNODES,NITB,np,npp,nq,nq2,nqq,nrow,ntemp,nzero,
     '  nzz,PLACEEXT,RMVDCOL
...
!     Diagnostics
      IF(DOP) THEN
        WRITE(OP_STRING,'(/'' Regions: '',3I4)') nr,nr2,nr3
        CALL WRITES(IODI,OP_STRING,ERROR,*9999)
        WRITE(OP_STRING,'(/'' Classes: '',3I4)') nx,nx2,nx3
        CALL WRITES(IODI,OP_STRING,ERROR,*9999)
        WRITE(OP_STRING,'(/'' GKs: '',2F12.6)') GK(1),GK2(1)
        CALL WRITES(IODI,OP_STRING,ERROR,*9999)
        WRITE(OP_STRING,'(/'' GQs: '',2F12.6)') GQ(1),GQ2(1)
        CALL WRITES(IODI,OP_STRING,ERROR,*9999)
      ENDIF
...
!     Create the D matrix
      CALL CPU_TIMER(CPU_USER,TIME_START2)
      nrow=NPNODE(0,nr2)+NPNODE(0,nr3)
      ncol=NPNODE(0,nr2)+NPNODE(0,nr3)+BCINNBND
      DO nq=1,nrow
        DO nq2=1,ncol
...
!     Create 1 matrix which can be multiplied by the D matrix
      ...
      DO nqq=(NPNODE(0,nr2)+1),(NPNODE(0,nr2)+NPNODE(0,nr3))
        ...
        NQNP_LOCAL(nqq)=NQNP(np)
      ENDDO
      CALL ISORTP(NQLIST(0),NQNP_LOCAL,NQNP_LOCAL_PIV)

      CALL ASSERT(nrow.EQ.NQLIST(0),' >>Matrix row size mismatch',
     '  ERROR,*9999)
      CALL ASSERT(ncol.EQ.NQLIST2(0),' >>Matrix col size mismatch',
     '  ERROR,*9999)

!     Pivot rows
      DO nqq=1,(NPNODE(0,nr2)+NPNODE(0,nr3))
        IF(NQNP_LOCAL_PIV(nqq).NE.nqq) THEN !must pivot
          DO nq2=nqq+1,NQLIST(0)
     ...
     '            nqq,NQNP_LOCAL_PIV(nqq)
                CALL WRITES(IODI,OP_STRING,ERROR,*9999)
              ENDIF
              DO nzero=1,(NPNODE(0,nr2)+NPNODE(0,nr3))
...
C$OMP&SHARED(D_MATRIX,GQ_D_MATRIX,GQGQ2,NPLIST,NQLIST,NQLIST2)
      DO nq=1,NQLIST(0)
        DO nq2=1,(NQLIST2(0)-NPLIST(0))
          ANS=0
          DO nzero=1,(NQLIST(0)-NPLIST(0))
...
!     Assemble grid coeffs
     ...
     '            COEFFSEXT(NQGP_PIVOT(nzero,nq))
            ...
            ELSE IF(nq.EQ.RMVDCOL) THEN
              !do nothing
              DO nzero=1,NQM
                YQ(nzero,1,1,nx2)=0.0d0
              ENDDO
              DO nzero=1,NQLIST(0)
                IF(nq.EQ.NQLIST(nzero)) nqq=nzero
              ENDDO
              DO nzero=1,NQLIST2(0)
                YQ(NQLIST2(nzero),1,1,nx2)=D_MATRIX(nqq,nzero)
              ENDDO
            ELSE
              DO nzero=1,NQLIST(0)
                IF(nq.EQ.NQLIST(nzero)) nqq=nzero
...
!     Diagnostics
     ...
     '  ELAPSED_TIME
      ...
      WRITE(OP_STRING,'(/'' Time for ASSEMBLE11_6 : '',F12.6)')
     '  ELAPSED_TIME
      CALL WRITES(IOOP,OP_STRING,ERROR,*9999)

      CALL EXITS('ASSEMBLE11_6')
      RETURN
 9999 CALL ERRORS('ASSEMBLE11_6',ERROR)
      CALL EXITS('ASSEMBLE11_6')
      RETURN 1
      END
