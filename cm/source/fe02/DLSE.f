      SUBROUTINE DLSE(IBT,IDO,nb,NEELEM,NLL,NNL,NPL,DL,SE,ERROR,*)

C#### Subroutine: DLSE
C###  Description:
C###    DLSE evaluates the element scale factors SE from the global line
C###    derivatives DL(1,nl) and DL(2,nl).
C***  NI1 is Xi number in cyclic order
C***  NKI(ni) is nk number of first deriv wrt Xi(ni)
...
!     Parameter List
     ...
     '  NPL(5,0:3,NLM)
      ...
      CHARACTER ERROR*(*)
!     Local Variables
      INTEGER N,na,NAE,nb,ne,NFTB,NI1(5,3),ni,ni2,ni3,
     '  NIB,NITB,nk,nk2,NKTB,NKI(3),nl,nn,nn2,NNK,noelem,nr,ns,NSG

      DATA NI1/1,0,0,0,0,
     '         1,2,1,0,0,
     '         1,2,3,1,2/

      CALL ENTERS('DLSE',*9999)
      NITB=NIT(nb)
      NKTB=NKT(0,nb)
      NFTB=1
      IF(IBT(1,NITB,nb).EQ.9) THEN !Fourier time basis
        NKTB=NKTB/IBT(2,NITB,nb)  !no. spatial derivs per node
        NFTB=IBT(2,NITB,nb)       !no. Fourier coeffs per spatial dof
        ...
        DO noelem=1,NEELEM(0,nr)
          ...
          IF(NKTB.GT.1.AND.NBI(nb).NE.1) THEN !scale factors not unit
            DO NAE=1,NLE(nb)
              ...
              IF(nl.GT.0) THEN
                ni=NPL(1,0,nl)
C AJP 4-6-93    IF(IBT(1,ni,nb).EQ.2) THEN !Hermite
c cbp 7/8/95 Extending sector bases
C                IF((IBT(1,ni,nb).EQ.2).OR.((IBT(1,ni,nb).EQ.3).AND.
C     '            (IBT(2,ni,nb).EQ.4))) THEN
C!                 !Hermite or special hermite simplex.
