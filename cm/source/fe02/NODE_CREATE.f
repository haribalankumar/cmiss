      SUBROUTINE NODE_CREATE(np,ERROR,*)

C#### Subroutine: NODE_CREATE
C###  Description:
...
C       Copy the existing data and zero the new data
     ...
     '    0,ERROR,*9999)
      ...
      ENDIF
      CALL NODE_CREATE_DYNAM(%VAL(CMGUI_NODE_PTR),np,ERROR,*9999)

      CALL EXITS('NODE_CREATE')
      RETURN
 9999 CALL ERRORS('NODE_CREATE',ERROR)
      CALL EXITS('NODE_CREATE')
      RETURN 1
      END
