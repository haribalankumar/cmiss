      SUBROUTINE VCD_INIT_GRID(NQLIST,CQ,RCQS,YQ,YQS,ERROR,*)

C#### Subroutine: VCD_INIT_GRID
C###  Description:
C###    VCD_INIT_GRID initialises arrays for the VCD
...
!     Local Variables
      ...
      CALL ENTERS('VCD_INIT_GRID',*9999)
...
!ICQS
      ...
      CELL_AII_OFFSET(1)=CELL_VARIANT_OFFSET
...
C        YQ(nq,NQLIST(3),1)=CQ(9,nq)
        ...
        YQS(CELL_STATE_OFFSET(1)+Recov-1,nq)=CQ(11,nq)
        YQS(CELL_STATE_OFFSET(1)+Cai-1,nq)=0.0d0
        YQS(CELL_STATE_OFFSET(1)+DVm-1,nq)=0.0d0
        YQS(CELL_STATE_OFFSET(1)+DRecov-1,nq)=0.0d0
!PARAMETERS
...
C        RCQS(CELL_PARAMETERS_OFFSET+Cm-1)=CQ(1,nq)*1.0d-6
        ...
        RCQS(CELL_PARAMETERS_OFFSET(1)+TConst-1)=CQ(10,nq)
        RCQS(CELL_PARAMETERS_OFFSET(1)+CaTConst-1)=CQ(12,nq)
        RCQS(CELL_PARAMETERS_OFFSET(1)+RepTConst-1)=CQ(14,nq)
!PROTOCOL
        RCQS(CELL_PROTOCOL_OFFSET(1)+PseudoIs-1)=0.0d0
        RCQS(CELL_PROTOCOL_OFFSET(1)+Is1current-1)=0.0d0
      ENDDO

      CALL EXITS('VCD_INIT_GRID')
      RETURN
 9999 CALL ERRORS('VCD_INIT_GRID',ERROR)
      CALL EXITS('VCD_INIT_GRID')
      RETURN 1
      END
