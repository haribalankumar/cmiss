      SUBROUTINE PF_QUADRATIC(CALL_FLAG,QTYPE,QUESTIONS,RVALUE,ERROR,*)

C#### Subroutine: PF_QUADRATIC
C###  Description:
C###    PF_QUADRATIC is a quadratic polynomial function y=ax^2+bx+c.
...
!      LOGICAL
      ...
      ELSEIF(CALL_FLAG(1:5).EQ.'EVALU') THEN
        RVALUE(1)=RVALUE(3)*RVALUE(2)**2+RVALUE(4)*RVALUE(2)+RVALUE(5)
      ENDIF

      CALL EXITS('PF_QUADRATIC')
      RETURN
 9999 CALL ERRORS('PF_QUADRATIC',ERROR)
      CALL EXITS('PF_QUADRATIC')
      RETURN 1
      END
