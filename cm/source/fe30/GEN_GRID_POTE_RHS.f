      SUBROUTINE GEN_GRID_POTE_RHS(IBT,IDO,INP,NBH,NBJ,NEELEM,NENQ,NHE,
     '  NHP,niqV,NKHE,NKH,NPF,NPNE,NPNODE,NQGP,NQGP_PIVOT,NQS,NQXI,
     '  nr_blood,nr_grid,nr_torso,NVHE,NVHP,NW,NWQ,nx_blood,nx_ext,
     '  nx_torso,nx_trans,nx_upd,NXQ,NYNE,NYNP,ALPHA,AQ,CQ,CURVCORRECT,
     '  DNUDXQ,DXDXIQ,DXDXIQ2,
     '  NQGW,RHS,SE,XIQ,YP,YQ,ZA,ZE,ZP,FIXQ,ERROR,*)

C#### Subroutine: GEN_GRID_POTE_RHS
C###  Description:
C###    GEN_GRID_POTE_RHS is used for coupled bidomain/torso problems
...
!     Parameter list
     ...
     '  NPNE(NNM,NBFM,NEM),NPNODE(0:NP_R_M,0:NRM),nr_grid,nr_torso,
     '  nr_blood,NQGP(0:NQGM,NQM),NQGP_PIVOT(NQGM,NQM),NQS(NEQM),
     ...
     '  nx_trans,nx_upd,nx_torso,nx_blood,nx_ext,NXQ(-NIM:NIM,0:4,0:NQM)
      REAL*8 ALPHA,AQ(NMAQM,NQM),CQ(NMM,NQM),CURVCORRECT(2,2,NNM,NEM),
     &  DNUDXQ(3,3,NQM),DXDXIQ(3,3,NQM),DXDXIQ2(3,3,NQM),RHS(NQM),
     &  NQGW(NQGM,NQM),SE(NSM,NBFM,NEM),XIQ(NIM,NQM),YP(NYM,NIYM,NXM),
     &  YQ(NYQM,NIQM,NAM,NXM),ZA(NAM,NHM,NCM,NEM),ZE(NSM,NHM),
     &  ZP(NKM,NVM,NHM,NPM,NCM)
      CHARACTER ERROR*(*)
      LOGICAL FIXQ(NYQM,NIYFIXM,NXM)
!     Local variables
      INTEGER i,ne,nh,ni,nq,NITB,nc,nb,nr,nx,nhx
      REAL*8 CMAMDT,FLUX,ONEMINUSALPHA,PXI,XI(3)
      REAL ELAPSED_TIME,TIME_START(1),TIME_STOP(1)
      LOGICAL ERROR_FLAG

      CALL ENTERS('GEN_GRID_POTE_RHS',*9999)
...
C$OMP PARALLEL DO
C$OMP&PRIVATE(CMAMDT,FLUX,i,nb,nc,ne,nh,nhx,ni,NITB,nq,nr,nx,XI,ZA,
C$OMP&  ZE,ZP)
...
C$OMP&  NQGP_PIVOT,NQGW,NQS,NQXI,nr_blood,nr_grid,nr_torso,NVHE,NVHP,
C$OMP&  NW,NWQ,NXQ,nx_blood,nx_ext,nx_torso,nx_trans,nx_upd,NYNE,NYNP,
        ...
        IF(.NOT.ERROR_FLAG) THEN
          IF(NWQ(1,nq).NE.0) THEN     !boundary grid point
            ...
            IF((nx.GT.0).AND.(nr.GT.0).AND.(ne.GT.0)) THEN
              IF(FIXQ(nq,1,nx_upd)) nc=1
              IF(FIXQ(nq,2,nx_upd)) nc=2
...
CC$OMP CRITICAL(GEN_GRID_POTE_RHS_2)
        WRITE(OP_STRING,'(1X,''Time to create RHS'',F6.2,''s cpu'')')
     '    ELAPSED_TIME
        CALL WRITES(IOOP,OP_STRING,ERROR,*9999)
CC$OMP END CRITICAL(GEN_GRID_POTE_RHS_2)
      ENDIF

      CALL EXITS('GEN_GRID_POTE_RHS')
      RETURN
 9999 CALL ERRORS('GEN_GRID_POTE_RHS',ERROR)
      CALL EXITS('GEN_GRID_POTE_RHS')
      RETURN 1
      END
