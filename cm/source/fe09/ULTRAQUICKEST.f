      REAL*8 FUNCTION ULTRAQUICKEST(nfv,NFVC,nh,NPNODE,nr,nvc,NVCNODE,
     '  NYNP,COUR,TC,TD,VC,XNFV,YP)

C#### Function: ULTRAQUICKEST
C###  Type: REAL*8
...
C###    tight resolution and accuracy) limiter and estimated streaming
C###    terms. It assumes that flow is from velocity TC to TD.
...
C###    pp 729-766 (1990).
      ...
      INCLUDE 'voro00.inc'
      REAL*8 CF,BIG
      PARAMETER (CF=0.166666666666666667D0,BIG=100.d0)
!     Parameter List
...
!     Local Variables
      ...
      REAL*8 DDOT,CURVN,DIST,TEMP,TU,T3,T4,T5,DMEDIAN
...
C         ..Compute normal component of curvature
     ...
     '      XNFV(1,nfv2),1,XNFV(1,nfv),1))
      ...
      T3=TU+BIG*(TC-TU)
      T4=DMEDIAN(TC,TD,T3)
      T5=0.5d0*(TC+TD)-0.5d0*COUR*(TD-TC)-CF*CURVN

      ULTRAQUICKEST=DMEDIAN(TC,T4,T5)
