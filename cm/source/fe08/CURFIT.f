...
C#### Subroutine: CURFIT
C###  Description:
C###    CURFIT fits a 2-D spline to a given set of data points. This
C###    routine and all subseqent calling routines were written by
C###    Paul Dierckx and are a subset of the FITPACK library available
C###    from http://www.netlib.org/dierckx/.  This routine is equivalent
C###    to the NAG E02BEF as it is written by the same author.
C###    These routines appear exactly as the author wrote
C###    them except the precision has been changed from REAL to REAL*8.
C###
C     These routines should be moved to a separate library
C     (?linear_solvers) as they are not developed in the University of
C     Auckland.  There is no explicit licence with these routines so it
C     should be made possible to build cm without the library.
C
...
c   x     : real*8*8 array of dimension at least (m). before entry, x(i)
...
c           factor. s >=0.0d0 unchanged on exit.
...
c  are invalid, control is immediately repassed to the calling program.
      ...
      DO 10 I=2,M
         IF(X(I-1).GE.X(I) .OR. W(I).LE.0.0d0) GO TO 50
  10  CONTINUE
