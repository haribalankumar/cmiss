      SUBROUTINE DEIMPO(STRING,ERROR,*)

C#### Subroutine: DEIMPO
C###  Description:
C###    DEIMPO defines parameters for importing.
...
C#### Command: FEM define import;l/p/r/w<;FILENAME[$current]><;(PATH/example)[$current]>
C###  Description:
C###    Define import parameters. Parameters can be read from or
C###    written to the file FILENAME.ipimpo, with $current specifing
...
C---------------------------------------------------------------------
      ...
      ELSE IF(CO(noco+1).EQ.'??') THEN
        CALL DOCUM('fe24','doc','DEIMPO',ERROR,*9999)
      ELSE
     ...
     '    CALCU,FILIO,GENER,MOUSE,STATUS,ERROR,*1)
        ...
        IF(FILIO) THEN
          ...
          DO WHILE(FIRST_TIME.OR.BACKUP)
            FIRST_TIME=.FALSE.
            CALL OPEN_FILE(ALL_REGIONS,IPFILE,FILE,'impo',STATUS,
     '        ERR,ERROR,*9999)
            CALL IPIMPO(ERROR,*9999)
