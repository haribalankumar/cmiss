      SUBROUTINE ZETX50(COORDS,CSTYPE,STRESSTYPE,IBT,IDO,INP,
...
C ***   Calculate DXNXI from inverse of DXIDN
...
C ***     Put partial derivs of deformed theta wrt undef Xj into DZDX
        ...
        ELSE IF(COORDS.EQ.'Fibre'.OR.COORDS.EQ.'Principal') THEN
          IF(ng.EQ.0) THEN
...
C              Critical section is not essential.
CC$        call mp_setlock()
          WRITE(OP_STRING,'('' RWX='',D12.4,'' RGX2D='',D12.4)')
     '      RWX,RGX2D
          CALL WRITES(IODI,OP_STRING,ERROR,*9999)
CC$        call mp_unsetlock()
        ENDIF
...
C          CALL F02ABF(TN,3,NITB,PST,RM,3,WK1_LOCAL,IFAIL)
C          DO ni=1,3
C            DO mi=1,3
C              RM(ni,mi)=TN(ni,mi)
C            ENDDO
...
C !!! TN is not symmetric so a different subroutine needs to be used to
C find the left eigenvectors.
C           CALL ESOLVE(.TRUE.,NITB,TN,3,PST,RM,ERROR,*9999)
          CALL WRITE_LINE(IOER,
     &      'WARNING: Principal nominal stresses are not implemented',
     &      ERROR,*9999)
          DO mi=1,3
            PST(mi)=0.d0
            DO ni=1,3
...
C          CALL F02ABF(TC,3,NITB,PST,RM,3,WK1_LOCAL,IFAIL)
C          DO ni=1,3
C            DO mi=1,3
C              RM(ni,mi)=TC(ni,mi)
C            ENDDO
