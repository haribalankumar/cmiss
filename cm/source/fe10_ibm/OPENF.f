...
C          STATUS_UNIX=CUPPER(STATUS)
     ...
     '      ACCESS='SEQUENTIAL',FORM=FORM,IOSTAT=IOSTAT)
        ...
        ELSE IF(ACCESS.EQ.'APPEND') THEN
          OPEN(UNIT=IUNIT,FILE=FILENAME(IBEG:IEND),STATUS=STATUS,
     '      ACCESS='SEQUENTIAL',POSITION='APPEND',FORM=FORM,
     '      IOSTAT=IOSTAT)
...
C               truncated name.
     ...
     '        FORM=FORM,RECL=IRECL,IOSTAT=IOSTAT)
      ...
      IF(IOSTAT.NE.0) THEN
        CALL FLAG_ERROR(0,' ')
        CALL WRITE_CHAR(IOER,'can''t open file ',ERR)
        CALL WRITE_STRING(IOER,LEN_TRIM(FILENAME),FILENAME,ERR)
        IF(IOSTAT.EQ.6) THEN
          CALL WRITE_CHAR(IOER,': file not found',ERR)
C         ELSE IF(IOSTAT.EQ.13) THEN
C           CALL STRING_TRIM(ERROR,IBEG,IEND)
C           ERROR=' Permission denied: '//ERROR(IBEG:IEND)
