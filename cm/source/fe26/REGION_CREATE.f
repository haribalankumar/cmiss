      SUBROUTINE REGION_CREATE(nr,ERROR,*)

C#### Subroutine: REGION_CREATE
C###  Description:
...
!     Local Variables
      ...
      CALL ENTERS('REGION_CREATE',*9999)
...
C       Copy the existing data and zero the new data
     ...
     '    0,ERROR,*9999)
      ...
      ENDIF
      CALL REGION_CREATE_DYNAM(%VAL(CMGUI_REGION_PTR),nr,ERROR,*9999)

      CALL EXITS('REGION_CREATE')
      RETURN
 9999 CALL ERRORS('REGION_CREATE',ERROR)
      CALL EXITS('REGION_CREATE')
      RETURN 1
      END
