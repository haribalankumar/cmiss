      SUBROUTINE MESH_BRANCH_CONTINUOUS(LD,LD_TEMP,numzero,nen,np1,np2,
     &  POINT_LIMIT,COFM,FRAC,A_LIM,L_LIM,LENGTH_PARENT,min_length,XP,
     &  XP1,ZD,BRANCH,SS,ERROR,*)
...
C***  Created by Merryn Howatson Tawhai, February 1997
      ...
      INCLUDE 'geom00.cmn'
      INCLUDE 'tol00.cmn'
      INTEGER LD(NDM),LD_TEMP(NDM),numzero,nen,np1,np2,POINT_LIMIT
     ...
     '  VECTOR(3)
      ...
      CALL ENTERS('MESH_BRANCH_CONTINUOUS',*9999)
      ...
      LENGTH=DSQRT(LENGTH)
      L_COFM=DSQRT(L_COFM)
      CALL NORMALISE(NJT,VECTOR,ERROR,*9999) !unit vector
...
C*** Check branch angle to parent
      CALL MESH_ANGLE_CHECK_CONTINUOUS(np1,np2,A_LIM,LENGTH,LU,XP,XP1,
     &  ERROR,*9999)
c      IF(LENGTH.GT.LENGTH_PARENT)THEN
c        DO nj=1,NJT
c          XP1(nj)=XP(1,1,nj,np1)+VECTOR(nj)*0.95d0*LENGTH_PARENT
c        ENDDO
c        IF(ngen.GT.10) BRANCH=.FALSE.
      ...
      CALL EXITS('MESH_BRANCH_CONTINUOUS')
      RETURN
 9999 CALL ERRORS('MESH_BRANCH_CONTINUOUS',ERROR)
      CALL EXITS('MESH_BRANCH_CONTINUOUS')
      RETURN 1
      END
