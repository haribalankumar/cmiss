      SUBROUTINE DOMSOLOPTI2D(IBT,IDO,INP,NBH,NBJ,NEELEM,NGAP,NHE,NHP,
     '  NKH,NKHE,NKJE,NLL,NPF,NP_INTERFACE,NPNE,NPNODE,nr,NVHE,NVHP,
     '  NVJE,NW,nx,NYNP,CE,CPCUTOFF,CURVCORRECT,DETLOC,DET_ADAPT,DL,
     '  DRDN,PG,PG_J,PG_Q,PG_U,RAD,RD,RG,SE,WG,XE,XG1,XIG,XIG_J,
     '  XIG_Q,XIG_U,XN,XP,XPFP,XR,YD,YP,ZA,ZE,ZF,ZP,ERROR,*)

C#### Subroutine: DOMSOLOPTI2D
C###  Description:
C###    DOMSOLOPTI2D calculates BE solution at the point XPFP but
C###    is based what could be! an optimised 2d laplace code. This
C###    version includes adaptive integration and logarithmic
C###    quadrature.
...
!     Parameter List
     ...
     '  NGAP(NIM,NBM),NHE(NEM),NHP(NPM),NKH(NHM,NPM,NCM),
     ...
     '  XR(NJM,NGM),YD,YP(NYM,NIYM),ZA(NAM,NHM,NCM,NEM),ZE(NSM,NHM),
     '  ZF(NSM,NHM),ZP(NKM,NVM,NHM,NPM,NCM)
      CHARACTER ERROR*(*)
!     Local Variables
      INTEGER GREENFLAG,intscheme,mk,nb,nb2,nbuh,nbuhp,nb1j,
     '  nb1jp,nbbem,nbqh,nbqhp,nc,ne,ng,nh,nhx,nk,nk1,nn,NNMIN,
     '  noelem,nonode,np,npnn,nrc,ns,NUMTYPES,nv,ny,type
      REAL*8 D(2),DDOT,DGREEN,DXXI(3,3),FACTOR,FACTOR1,GREEN,MINDIST,
     '  SUMC,SUMU,SUMQ,VLENGTH,XIMIN(2)
      LOGICAL INTERFACE,USEADAPINT,USELOGBEM

      CALL ENTERS('DOMSOLOPTI2D',*9999)
...
C***        Calculate gauss point dependent arrays
            DO ng=1,NGT(nb1j)
              ...
              DRDN(ng)=(XR(1,ng)*XN(1,ng)+XR(2,ng)*XN(2,ng))/RAD(ng)
            ENDDO !ng
...
C***        GK matrix components
            IF(GREENFLAG.NE.2) THEN
              ns=0
              DO ng=1,NGT(nbuh)
                DGREEN=-DET_ADAPT(nbuhp,NNMIN,ng)*DRDN(ng)/RAD(ng)
...
C***        GQ matrix components
            IF(GREENFLAG.EQ.1) THEN
              IF(NNMIN.EQ.1) THEN
                DO ng=1,NGT(nbqh)
                  GREEN=-DET_ADAPT(nbuhp,NNMIN,ng)*
     '              DLOG(RAD(ng)/XIG(1,ng,nbqh))
            ...
            ELSE
              DO ng=1,NGT(nbqh)
                GREEN=-DET_ADAPT(nbuhp,NNMIN,ng)*DLOG(RAD(ng))
                RD(ng)=GREEN*RG(ng)*WG(ng,nbqh)
              ENDDO !ng
            ENDIF
...
C***        GK matrix components
            IF(GREENFLAG.NE.2) THEN
              ns=0
              DO ng=1,NGT(nbuh)
                DGREEN=-DETLOC(nbuhp,NNMIN,ng,1)*DRDN(ng)/RAD(ng)
                RD(ng)=DGREEN*RG(ng)*WG(ng,nbuh)
                SUMC=SUMC-RD(ng)/FACTOR
              ENDDO !ng
              DO nn=1,NNT(nbuhp) !Dependent variable loop
...
C***        GQ matrix components
     ...
     '              DLOG(RAD(ng)/XIG(1,ng,nbqh))
              ...
              ELSE IF(NNMIN.EQ.2) THEN
                DO ng=1,NGT(nbqh)
                  GREEN=DETLOC(nbuhp,NNMIN,ng,1)*
     '              DLOG(RAD(ng)/(1.0d0-XIG(1,ng,nbqh)))
                  RD(ng)=GREEN*RG(ng)*WG(ng,nbqh)
                ENDDO !ng
              ENDIF
            ELSE
              DO ng=1,NGT(nbqh)
                GREEN=-DETLOC(nbuhp,NNMIN,ng,1)*DLOG(RAD(ng))
                RD(ng)=GREEN*RG(ng)*WG(ng,nbqh)
              ENDDO !ng
            ENDIF
     ...
     '            KTYP93(2,nr),1)) THEN
      ...
      CALL EXITS('DOMSOLOPTI2D')
      RETURN
 9999 CALL ERRORS('DOMSOLOPTI2D',ERROR)
      CALL EXITS('DOMSOLOPTI2D')
      RETURN 1
      END
