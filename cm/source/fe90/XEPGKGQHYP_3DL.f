      SUBROUTINE XEPGKGQHYP_3DL(nb1j,nb1jp,nbqh,nbqhp,nbuh,
     '  nbuhp,NDTOT,ne,nh,NHSTGK,
     '  NHSTGQ,NKHE,NKH,np,NPNE,nr,CE,CURVCORRECT,DET,
     '  DRDNO,DSDX,GKES,GQES,PG_Q,PG_U,RAD,RG,SE,WG,XG1,
     '  XN,XNO,XN_GRAD,XPFP,XR,XR_GRAD,CALC_XR,ERROR,*)
C SMAR009 19/01/99 removed GK,GQ,ISC_GK,ISC_GQ,ISR_GK,ISR_GQ,nx,NYNP,
C#### Subroutine: XEPGKGQHYP_3DL
C###  Description:
C###    XEPGKGQHYP_3DL is an efficient version of XEPGKGQHYP for 3D
C###    Laplace equation in rectangular cartesian coordinates.
C###  See-Also: XEPGKGQHYP
...
!     Parameter List
     ...
     '  NKH(NHM,NPM,NCM),np,NPNE(NNM,NBFM),nr
...
C  NYNP(NKM,NVM,NHM,NPM,0:NRCM,NCM,NRM)
     ...
     '  RAD(NGM),RG(NGM),
     '  SE(NSM,NBFM,NEM),XG1(NJM,NUM,NGM),XN(NJM,NGM),XNO(3,*),
     '  XN_GRAD(NJM,NGM),XPFP(*),XR(NJM,NGM),XR_GRAD(NJM,NGM),
     '  WG(NGM,NBM)
...
!     Local Variables
      ...
      REAL*8 DGREEN,FACTOR,FACTOR1,FACTOR2,HYPGREEN,NDOTNO,RDOTN,RDOTNO,
     '  SUM1,SUM2,SUM3(3),SUMQ,TERM(999),TERM1

      CALL ENTERS('XEPGKGQHYP_3DL',*9999)
...
c cpb 9/7/95 Inlining hypgreen
C          TERM1=RG(ng)*WG(ng,nb1j)*HYPGREEN(IGREN(nr),0,CE,RAD(ng),
C     '         DET(nb1jp,NNMIN,ng),XN(1,ng),XNO(1,nd),XR(1,ng))
C          TERM2=RG(ng)*WG(ng,nb1j)*HYPGREEN(IGREN(nr),1,CE,RAD(ng),
C     '         DET(nb1jp,NNMIN,ng),XN(1,ng),XNO(1,nd),XR(1,ng))
     ...
     '      DET(nb1jp,0,ng)/(RAD(ng)*RAD(ng)*RAD(ng))
          TERM1=RG(ng)*WG(ng,nb1j)*HYPGREEN
          SUM1=SUM1+TERM1
c cpb 9/7/95 unrolling loops
C          DO nj=1,NJ_LOC(NJL_GEOM,0,nr)-1
C            SUM3(nj)=SUM3(nj)+TERM1*XR_GRAD(nj,ng)
C          ENDDO
C          SUM3(NJ_LOC(NJL_GEOM,0,nr))=SUM3(NJ_LOC(NJL_GEOM,0,nr))
C     '      +TERM2*XR_GRAD(NJ_LOC(NJL_GEOM,0,nr),ng)
          SUM3(1)=SUM3(1)+TERM1*XR_GRAD(1,ng)
          SUM3(2)=SUM3(2)+TERM1*XR_GRAD(2,ng)
          SUM3(3)=SUM3(3)+TERM1*XR_GRAD(3,ng)
        ENDDO !End of ng loop
        SUM1=SUM1/FACTOR
        SUM3(1)=SUM3(1)/FACTOR
        SUM3(2)=SUM3(2)/FACTOR
        SUM3(3)=SUM3(3)/(FACTOR1*FACTOR2)
...
C        CALL SPARSE(ny1,NYP2,NYT(1,1,nx),nz,NZ_GK_M,NZT(1,nx),
C     '    ISC_GK,ISR_GK,KTYP24,ERROR,*9999)
C        GK(nz)=GK(nz)-SUM3(1)
        GKES(-2,nk)=GKES(-2,nk)-SUM3(1)
C        NYP2=NYNP(3,1,nh,np,2,1,nr) !local column number of GK
C        CALL SPARSE(ny1,NYP2,NYT(1,1,nx),nz,NZ_GK_M,NZT(1,nx),
C     '    ISC_GK,ISR_GK,KTYP24,ERROR,*9999)
C        GK(nz)=GK(nz)-SUM3(2)
        GKES(-3,nk)=GKES(-3,nk)-SUM3(2)
...
C        GQ(nz)=GQ(nz)+SUM3(3)
        GQES(-1,nk)=GQES(-1,nk)+SUM3(3)
        !GK matrix components
        ns=0
        nhs=0
C        ny1=NYNP(nk,1,nh,np,1,1,nr) !eqtn number (row number) of GK.
c cpb 10/7/95 Precalc HYGREEN*RG*WG
        DO ng=1,NGT(nbuh)
          NDOTNO=XN(1,ng)*XNO(1,nd)+XN(2,ng)*XNO(2,nd)+XN(3,ng)*
     '      XNO(3,nd)
          RDOTN=XR(1,ng)*XN(1,ng)+XR(2,ng)*XN(2,ng)+XR(3,ng)*XN(3,ng)
...
C              ny2=NYNP(MK,1,nh,npnn,2,1,nr) !local column number of GK
              ...
              DO ng=1,NGT(nbuh)
c cpb 9/7/95 Inlining hypgreen
C                SUM2=SUM2+RG(ng)*HYPGREEN(IGREN(nr),0,CE,RAD(ng),
C     '               DET(nbuhp,NNMIN,ng),XN(1,ng),XNO(1,nd),XR(1,ng))*
C     '               PG(ns,1,ng,nbuh)*WG(ng,nbuh)
                SUM2=SUM2+TERM(ng)*PG_U(ns,1,ng)
              ENDDO !End of ng loop
C              CALL SPARSE(ny1,ny2,NYT(1,1,nx),nz,NZ_GK_M,
C     '          NZT(1,nx),ISC_GK,ISR_GK,KTYP24,ERROR,*9999)
C              GK(nz)=GK(nz)+SUM2*SE(ns,nbuhp,ne)/FACTOR
              GKES(nhs,nk)=GKES(nhs,nk)+SUM2*SE(ns,nbuhp,ne)/FACTOR
            ENDIF
...
C          TERM1=DGREEN(IGREN(nr),0,nh,nh,CE,DRDNO(ng,nd),RAD(ng),
C     '         DET(nb1jp,NNMIN,ng),XN(1,ng),XR(1,ng))*
C     '         RG(ng)*WG(ng,nb1j)
C          TERM2=DGREEN(IGREN(nr),1,nh,nh,CE,DRDNO(ng,nd),RAD(ng),
C     '         DET(nb1jp,NNMIN,ng),XN(1,ng),XR(1,ng))*
C     '         RG(ng)*WG(ng,nb1j)
          DGREEN=-DRDNO(ng,nd)*DET(nb1jp,0,ng)/(RAD(ng)*RAD(ng))
          TERM1=DGREEN*RG(ng)*WG(ng,nb1j)
c cpb 10/7/95 unrolling loops
C          DO nj=1,NJ_LOC(NJL_GEOM,0,nr)-1
C            SUM3(nj)=SUM3(nj)+TERM1*XN_GRAD(nj,ng)
C          ENDDO
C          SUM3(NJ_LOC(NJL_GEOM,0,nr))=SUM3(NJ_LOC(NJL_GEOM,0,nr)
C     '      )+TERM2*XN_GRAD(NJ_LOC(NJL_GEOM,0,nr),ng)
          SUM3(1)=SUM3(1)+TERM1*XN_GRAD(1,ng)
          SUM3(2)=SUM3(2)+TERM1*XN_GRAD(2,ng)
          SUM3(3)=SUM3(3)+TERM1*XN_GRAD(3,ng)
        ENDDO !End of ng loop
        SUM3(1)=SUM3(1)/FACTOR
        SUM3(2)=SUM3(2)/FACTOR
        SUM3(3)=SUM3(3)/(FACTOR1*FACTOR2)
...
C              GQ(nz)=GQ(nz)+SUMQ*SE(ns,nbqhp,ne)/(FACTOR1*FACTOR2)
              GQES(nhs,nk)=GQES(nhs,nk)+SUMQ*SE(ns,nbqhp,ne)/
     '          (FACTOR1*FACTOR2)
...
C                CALL SPARSE(ny1,ny2,NYT(1,1,nx),nz,NZ_GK_M,
C     '            NZT(1,nx),ISC_GK,ISR_GK,KTYP24,ERROR,*9999)
C                GK(nz)=GK(nz)-SUMQ*SE(ns,nbqhp,ne)/FACTOR1*
...
C     '            CURVCORRECT(2,nk1-1,nn)
                GKES(nsnn+2,nk)=GKES(nsnn+2,nk)-SUMQ*SE(ns,nbqhp,ne)/
     '            FACTOR1*CURVCORRECT(1,nk1-1,nn)
                GKES(nsnn+3,nk)=GKES(nsnn+3,nk)-SUMQ*SE(ns,nbqhp,ne)/
     '            FACTOR1*CURVCORRECT(2,nk1-1,nn)
              ENDIF
            ENDIF
          ENDDO !End of nk1 loop
        ENDDO !End of nn loop
...
CC$OMP CRITICAL(XEPGKGQHYP_3DL_1)
...
C          ENDDO !nn
     ...
     '        nhs=-NDTOT-1,NHSTGQ)
            CALL WRITES(IODI,OP_STRING,ERROR,*9999)
          ENDDO !nkk
CC$OMP END CRITICAL(XEPGKGQHYP_3DL_1)
        ENDIF !dop
      ENDDO !End of nk loop

      CALL EXITS('XEPGKGQHYP_3DL')
      RETURN
 9999 CALL ERRORS('XEPGKGQHYP_3DL',ERROR)
      CALL EXITS('XEPGKGQHYP_3DL')
      RETURN 1
      END
