      SUBROUTINE DIST_LOOSE(intscheme,nb,NLL,nnmin,NPNE,nr,NW,DL,
     '  MINDIST,XP,XPFP,ERROR,*)

C#### Subroutine: DIST_LOOSE
C###  Description:
C###    DIST_LOOSE determines the minimum distance (MINDIST) from the
...
C###    based on this minimum distance. This routine checks against
C###    LOOSE_TOL as opposed to RDELTA for element splitting.
...
C*** Transform to cartesian to find distance
        ...
        ENDDO !nn
      ENDIF
      IF(MINDIST.LE.LOOSE_TOL) THEN !Node within the element
...
C*** Average side length in xi1 and xi2 direction
     ...
     '        ERROR,*9999)
      ...
      CALL EXITS('DIST_LOOSE')
      RETURN
9999  CALL ERRORS('DIST_LOOSE',ERROR)
      CALL EXITS('DIST_LOOSE')
      RETURN 1
      END
