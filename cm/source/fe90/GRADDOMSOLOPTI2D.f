      SUBROUTINE GRADDOMSOLOPTI2D(NBH,NBJ,NEELEM,NHE,NHP,NKH,NKHE,NKJE,
     '  NLL,NPF,NP_INTERFACE,NPNE,NPNODE,nr,NVHE,NVHP,NVJE,NW,nx,
     '  NYNE,NYNP,CE,CURVCORRECT,DET,DL,DRDN,GRADPHI,PG,RAD,RG,SE,
     '  WG,XE,XG1,XN,XP,XPFP,XR,YP,ZA,ZE,ZF,ZP,ERROR,*)

C#### Subroutine: GRADDOMSOLOPTI2D
C###  Description:
C###    GRADDOMSOLOPTI2D calculates the gradient of the potential at
...
!     Parameter List
     ...
     '  ZE(NSM,NHM),ZF(NSM,NHM),ZP(NKM,NVM,NHM,NPM,NCM)
      CHARACTER ERROR*(*)
!     Local Variables
      INTEGER intscheme,MK,nb,nbuh,nbuhp,nb1j,nb1jp,nbbem,nbqh,nbqhp,ne,
     '  ng,nh,nhx,nj,nj_grad,nk1,nn,NNMIN,noelem,np,npnn,ns
      REAL*8 DDOT,DGREEN,DXXI(3,3),HYPGREEN,MINDIST,SUMC,SUMQ,
     '  SUMU,VLENGTH,XNO(3)
      LOGICAL DUMMY,INTERFACE

      CALL ENTERS('GRADDOMSOLOPTI2D',*9999)
...
C***      Extra calc's to get sumc
          ...
          SUMC=SUMC+((DET(nbuhp,NNMIN,ng,1)*DRDN(ng)*RG(ng)*
     '      WG(ng,nbuh))/(RAD(ng)*2.0d0*PI))
...
C***  Only want gradients that are definitely internal
      ...
      CALL EXITS('GRADDOMSOLOPTI2D')
      RETURN
9999  CALL ERRORS('GRADDOMSOLOPTI2D',ERROR)
      CALL EXITS('GRADDOMSOLOPTI2D')
      RETURN 1
      END
