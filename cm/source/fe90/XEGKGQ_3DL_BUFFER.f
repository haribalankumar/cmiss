      SUBROUTINE XEGKGQ_3DL_BUFFER(IBT,IDO,INP,ISC_GK,ISC_GKK,ISC_GQ,
     '  ISR_GK,ISR_GKK,ISR_GQ,LGKE,LGQE,NBH,NBJ,NDET,ne,NENP,NGAP,NKH,
     '  NKHE,NKJE,NLL,NONY,NPB,NP_INTERFACE,NPF,NPNE,NPNODE,NPNY,nr,
...
C#### Subroutine: XEGKGQ_3DL_BUFFER
C###  Description:
C###    XEGKGQ_3DL_BUFFER is the buffered efficient version of XEGKGQ
C###    for 3d Laplace's equation in rectangular cartesian coordinates.
C###  See-Also: XEGKGQ_3DL
...
!     Parameter List
     ...
     '  RAD(NGM),RG(NGM),SE(NSM,NBFM,NEM),WG(NGM,NBM),
...
!     Local Variables
     ...
     '  nbqh,nbqhp,nbuh,nbuhp,NDTOT,ng,nh,nhs,NHSTGK,NHSTGQ,nkk,
     '  NNSPLIT(20),NNMIN,NNSP,no1,no2,noy1,noy2,nonode,np,NSPLIT,
     '  ny1,ny2,ny3,nz,nzz
      REAL*8 co1,co2,D(2),DDOT,DSDX(3,3),DXDS(3,3),DXXI(3,3),VLENGTH,
     '  XIMIN(2),XNO(3,4),XPFP(3)
      LOGICAL GEN_CONVENT_EQUATION,GEN_DERIV_EQUATION,CALC_XR,USEADAPINT

      CALL ENTERS('XEGKGQ_3DL_BUFFER',*9999)

      IF(DOP) THEN
C KAT 20Mar01: Can't branch out of critical section.
C              Critical section is not essential.
CC$OMP CRITICAL(XEGKGQ_3DL_BUFFER_1)
        WRITE(OP_STRING,'(/'' Element number '',I5)') ne
        CALL WRITES(IODI,OP_STRING,ERROR,*9999)
CC$OMP END CRITICAL(XEGKGQ_3DL_BUFFER_1)
      ENDIF
...
C KAT 20Mar01: Can't branch out of critical section.
C              Critical section is not essential.
CC$OMP CRITICAL(XEGKGQ_3DL_BUFFER_2)
          WRITE(OP_STRING,'(/'' Integration scheme '',I1,'', nbbem '','
     '     //'I2)') intscheme,nbbem
          CALL WRITES(IODI,OP_STRING,ERROR,*9999)
CC$OMP END CRITICAL(XEGKGQ_3DL_BUFFER_2)
        ENDIF
...
C KAT 20Mar01: Can't branch out of critical section.
C              Critical section is not essential.
CC$OMP CRITICAL(XEGKGQ_3DL_BUFFER_3)
            WRITE(OP_STRING,'(/'' Node number '',I5)') np
            CALL WRITES(IODI,OP_STRING,ERROR,*9999)
CC$OMP END CRITICAL(XEGKGQ_3DL_BUFFER_3)
          ENDIF
          CALC_XR=.TRUE.
          XPFP(1)=XP(1,1,1,np)
          XPFP(2)=XP(1,1,2,np)
...
C                  DO ni=1,NITB
C                    DO nj=1,NJ_LOC(MJL_GEOM,0,nr)
...
C*** XEGKGQ calculates the standard BIE equations.  It uses
C*** ny1=nynp(1,1,nh,1,np,nrc,nc,nr) i.e. it assumes nk=1.
              IF(USEADAPINT) THEN
                CALL XEPGKGQ_3DL(nb1j,
     '            nbqh,nbqhp,nbuh,nbuhp,NDTOT,nh,NHSTGK,NHSTGQ,
     '            NKHE(1,1,1,ne),NKH,NNMIN,np,NPNE(1,1,ne),nr,
     '            CE(1,ne),CURVCORRECT(1,1,1,ne),DET_ADAPT,
     '            DRDN,GKES,GQES,PG_Q,PG_U,RAD,RG,SE(1,1,ne),
     '            WG,XG1,XN,XPFP,XR,CALC_XR,ERROR,*9999)
...
C*** The routine XEGKGQHYP evaluates the derivative BIE(s) for the
...
C SMAR009 19/01/99 removed GK,GQ,ISC_GK,ISC_GQ,ISR_GK,ISR_GQ,nx,NYNP,
     ...
     '              GQES,PG(1,1,1,nbqh),PG(1,1,1,nbuh),RAD,
     '              RG,SE,WG,XG1,XN,XNO,XN_GRAD,XPFP,XR,XR_GRAD,
     '              CALC_XR,ERROR,*9999)
C SMAR009 19/01/99 removed GK,GQ,ISC_GK,ISC_GQ,ISR_GK,ISR_GQ,nx,NYNP,
                ENDIF
...
C*** derivative equations are calculated for each value of nk.
     ...
     '              GKES,GQES,PG_Q,PG_U,RAD,RG,SE(1,1,ne),WG,XG1,
     '              XN,XNO,XN_GRAD,XPFP,XR,XR_GRAD,CALC_XR,ERROR,*9999)
C SMAR009 19/01/99 removed GK,GQ,ISC_GK,ISC_GQ,ISR_GK,ISR_GQ,nx,NYNP,
                ELSE
                  CALL XEPGKGQHYPS_3DL(nb1j,nb1jp,nbqh,nbqhp,
     '              nbuh,nbuhp,NDTOT,nh,
     ...
     '              NPNE(1,1,ne),nr,CE(1,ne),
     '              CURVCORRECT(1,1,1,ne),DET(1,0,1,nsplit+IADAPTIVE),
     '              DRDNO,DSDX,GKES,GQES,
     '              PG(1,1,1,nbqh),PG(1,1,1,nbuh),
...
C$OMP CRITICAL(XEGKGQ_3DL_BUFFER_4)
            DO nkk=1,NDTOT+1
              DO nhs=-NDTOT-1,-1
                nz=LGKE(nhs,nkk,2)
              ...
              ENDDO !nhs
            ENDDO !nkk
C$OMP END CRITICAL(XEGKGQ_3DL_BUFFER_4)
          ELSE
C KAT 20Mar01: Can't branch out of critical section.
C              Atomic directive is probably better anyway.
CC$OMP CRITICAL(XEGKGQ_3DL_BUFFER_5)
            DO nkk=1,NDTOT+1
              ny1=LGKE(0,nkk,1)
...
C$OMP               ATOMIC
              ...
              ENDIF
            ENDDO !nkk
CC$OMP END CRITICAL(XEGKGQ_3DL_BUFFER_5)
          ENDIF
        ENDDO !End of np loop
      ENDDO !End of integration scheme loop

      CALL EXITS('XEGKGQ_3DL_BUFFER')
      RETURN
 9999 CALL ERRORS('XEGKGQ_3DL_BUFFER',ERROR)
      CALL EXITS('XEGKGQ_3DL_BUFFER')
      RETURN 1
      END
