...
C###  Description:
C###    POLYMARKER draws a polymarker on iw with index IBUNDLE.
...
C        ENDDO
      ...
      IF(WINDOW_TYPE(1:5).EQ.'MOTIF') THEN
        DO nopts=1,NTPTS
          R_XPTS(nopts)=REAL(D_PTS(NJ1,nopts))   !is real x
          R_YPTS(nopts)=REAL(D_PTS(NJ2,nopts))   !is real y
        ENDDO

        CALL POLYMARKER_GX(INDEX,NTPTS,R_XPTS,R_YPTS,ERROR,*9999)
      ENDIF
