      SUBROUTINE POLYLINE_DYNAM(IBUNDLE,iw,LDRPTS,NTPTS,R_PTS,D_PTS,
     '  ERROR,*)

C#### Subroutine: POLYLINE_DYNAM
C###  Description:
C###    POLYLINE_DYNAM draws a polyline on iw with index IBUNDLE. It
C###    differs from POLYLINE in that the REAL data array for GX is
...
!     Local Variables
      ...
      CALL ENTERS('POLYLINE_DYNAM',*9999)
      ...
      IF(DOP) THEN
        WRITE(OP_STRING,
     '    '('' iw='',I2,'' LDRPTS='',I6,'' NTPTS='',I6)') iw,LDRPTS,
     '    NTPTS
 ...
 9998 CALL EXITS('POLYLINE_DYNAM')
      RETURN
 9999 CALL ERRORS('POLYLINE_DYNAM',ERROR)
      CALL EXITS('POLYLINE_DYNAM')
      RETURN 1
      END
