...
!     Local Variables
     ...
     &  xidir(1:6),nu,nured,nk,NKJCHEK,NKJNEED
      REAL*8 PXI,SQND,SQNDlast,XIlast(3),FDSQ(0:NFM),
     &  SUM,DSDXI(3),DLAMCH,DIFF,TOLER
     ...
     &      XA(1,1,ne),XE,XP,ERROR,*9999)
          ...
          IF(INLIST(np,NPLIST(1),NPLIST(0),N1LIST))THEN
            ...
            IF(nb.GT.0) THEN
             ...
             DO nu=1,NUT(nb)
            Xmod(nu,nj)=PXI(IBT(1,1,nb),IDO(1,1,0,nb),INP(1,1,nb),
     &      nb,nu,XI,XE(nv,nj))
             ENDDO !nu
             ENDDO !nv 
            ENDIF !nb
          ...
          DO ni=1,NIT(NBJ(1,ne))
           ...
           SUM=SUM+Xmod(nured,3)**2
            ENDIF !njt
             DSDXI(ni)=SQRT(SUM)
          TOLER=DSQRT(DLAMCH('EPS'))
          IF(DABS(DSDXI(ni)).LT.TOLER) THEN
            DSDXI(ni)=1.0d0
            WRITE(OP_STRING,
...
!               arc-length or ave. arc-length scale factors
        ...
        NEWVERSION(nj)=.TRUE.
        IF((nv.lt.NVJP(nj,np)).and.(NEWVERSION(nj))) THEN
          NEWVERSION(nj)=.FALSE.
          NKJNEED=NKT(nn,nb)
          NKJCHEK=MIN(nk,NKJNEED)
         DO nk=1,NKJNEED
          IF((nk.lt.NKJCHEK).and.(.not.NEWVERSION(nj))) THEN
          DIFF=DABS(Xmod(nu,nj))-XP(nk,nv,to_field(nj),np)
            IF(DIFF.gt.(TOLER*DABS(Xmod(nu,nj))+TOLER)) THEN
            NEWVERSION(nj)=.TRUE.
            ENDIF!DIFF
          ENDIF !nk
         ENDDO !nk
