      SUBROUTINE HIDATA(ISDANO,ISDAPR,ISDATA,ISDATR,ISEG,NEELEM,
     '  STRING,ERROR,*)

C#### Subroutine: HIDATA
C###  Description:
C###    HIDATA hides data segments.
      ...
      INCLUDE 'ntsg00.cmn'
!     Parameter List
      INTEGER ISEG(*),ISDANO(NWM,NEM),
     '  ISDAPR(NWM,NEM),ISDATA(NWM,NGRSEGM),
     '  ISDATR(NWM,NEM),NEELEM(0:NE_R_M,0:NRM)
...
!     Local Variables
     ...
     '  noiw,nr,NTIW
      ...
      CALL ENTERS('HIDATA',*9999)
      IF(CO(noco+1).EQ.'?') THEN
        CALL STRING_TRIM(STRING,IBEG,IEND)

C---------------------------------------------------------------------

C#### Command: FEM hide data
C###  Description:
C###    Hide specified data on specified workstations.
C###  Parameter:    <(positions/numbers/projections/trace)[positions]>
C###    Specify what type of data information is to be hidden.
C###  Parameter:    <at (DATA_SET#/last)[last]>
C###    Specify which data sets are to be hidden. The options are
C###    all data sets or just the last data set.
C###  Parameter:    <on (all/WS#s)[all]>
C###    Specify which windows to hide the data sets on. The
C###    default is to hide data in all windows.
...
C---------------------------------------------------------------------
      ...
      ELSE IF(CO(noco+1).EQ.'??') THEN
        CALL DOCUM('fe22','doc','HIDATA',ERROR,*9999)
      ELSE
        ...
        DO noiw=1,NTIW
          ...
          IF(IWKS(iw).GT.0) THEN
            ...
            IF(TYPE(1:8).EQ.'POSITION') THEN
              ...
              IF(ISD.GT.0.AND.ISEG(ISD).EQ.2) THEN
                CALL VISIB(iw,ISEG,ISD,'INVISIBLE',ERROR,*9999)
              ELSE IF(ISD.EQ.0.OR.ISEG(ISD).EQ.0) THEN
                WRITE(OP_STRING,'('' >>Data at '',I3,'
     '            //''' is not defined on '',I1)') nodata,iw
