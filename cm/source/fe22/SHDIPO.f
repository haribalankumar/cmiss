      SUBROUTINE SHDIPO(DIPOLE_LIST,ISEG,ISDIPO,ISDIPA,NDIPOLES,NRLIST,
     '  NXLIST,STRING,ERROR,*)

C#### Subroutine: SHDIPO
C###  Description:
C###    SHDIPO shows dipole segments.
...
!     Local Variables
     ...
     '  IWK(6),N3CO,n,noiw,no_n,no_nrlist,nr,NTIW,nx,nxc
      ...
      CALL ENTERS('SHDIPO',*9999)
      IF(CO(noco+1).EQ.'?') THEN
        CALL STRING_TRIM(STRING,IBEG,IEND)
C---------------------------------------------------------------------

C#### Command: FEM show dipole
C###  Description:
C###    Make dipole segments visible on specified workstations.
C###  Parameter:    <on (all/WS#s)[all]>
C###    Specify which windows to show the dipole segments on.
C###    The default is to show the dipole segments on all windows.
C###  Parameter:    <dipole (all/DIPOLE#)[all]>
C###    This parameter allows you to sepcify to show either an
C###    individual dipole or all dipoles.
C###  Parameter:    <(vector/path)[vector]>
C###    Specify whether the dipole vectors are to be visible or
C###    the dipole paths are to be visible.
...
C---------------------------------------------------------------------
      ...
      ELSE IF(CO(noco+1).EQ.'??') THEN
        CALL DOCUM('fe22','doc','SHDIPO',ERROR,*9999)
      ELSE
              ...
              DO no_n=1,DIPOLE_LIST(0)
                ...
                IF(VECTOR) THEN
                  IF(ISDIPO(iw,n,nr).GT.0) THEN
                    CALL VISIB(iw,ISEG,ISDIPO(iw,n,nr),'VISIBLE',
     '                ERROR,*9999)
                  ENDIF
                ELSE IF(PATH) THEN
                  IF(ISDIPA(iw,n,nr).GT.0) THEN
                    CALL VISIB(iw,ISEG,ISDIPA(iw,n,nr),'VISIBLE',
     '                ERROR,*9999)
                  ENDIF
                ENDIF
              ENDDO
