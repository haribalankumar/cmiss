/*
triangulate          Computes a Delaunay triangulation
*/

/*  DESCRIPTION :                                                            */
/*  This module fetriangle.c contains the source code for the program        */
/*  Triangle Version 1.3 and its header file.                                */
/*  The interface to triangulate() has been modified to be called from       */
/*  fortran 77, and use CMISS data stuctures XP and NPNE.                    */
/*  All functions have had CMISS style error handling added.                 */
/*  In the interest of making this module meet the CMISS requirements, all   */
/*  calls to triangulate should use the -Q flag to supress the screen output.*/

/*  The header of Triangle Version 1.3 contains an excellent explanation     */
/*  of the passed parameters, although they are no longer in the structure   */
/*  described.                                                               */

/*  CREATED : Carey Stevens July 1996                                        */
...
/*                  struct triangulateio *,int *ERROR_FLAG, char *ERROR_STRINGC); */
...
int poly, refine, quality, vararea, fieldarea, fixedarea, regionattrib, convex;
int firstnumber;
...
void parsecommandline(argc, argv, ERROR_FLAG, ERROR_STRINGC)
int argc;
char **argv;
int *ERROR_FLAG;
char *ERROR_STRINGC;
        ...
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            if (maxarea <= 0.0) {
              ...
              *ERROR_FLAG=1;
              strcpy(ERROR_STRINGC,">>Maximum area must be greater than zero>parsecommandline");
              return;
	    }
	  } else {
            vararea = 1;
...
/*****************************************************************************/
/*                                                                           */
/*  set_flags()   identify switches, and set up options                      */
/*                                                                           */
/*  The effects of this routine are felt entirely through global variables.  */
/*                                                                           */
/*  Used in place of parsecommandline() for calling from CMISS               */
/*  Created by Carey Stevens 26/9/97                                         */
/*                                                                           */
/*                                                                           */
/*****************************************************************************/

void set_flags(FLAGS, ERROR_FLAG, NUM_STEINER_POINTS, MAXIMUM_AREA, MINIMUM_ANGLE, 
  ERROR_STRINGC)
int *FLAGS;
int *ERROR_FLAG;
int *NUM_STEINER_POINTS;
...
char *ERROR_STRINGC;
{
  ...
  poly = refine = quality = vararea = fieldarea = fixedarea = regionattrib = convex = 0;
  firstnumber = 1;
  edgesout = voronoi = neighbors = geomview = 0;
  nobound = nopolywritten = nonodewritten = noelewritten = noiterationnum = 0;
  noholes = noexact = 0;
  ...
  if (flag[3]) {
    ...
    if (maxarea <= 0.0) {
      *ERROR_FLAG=1;
      strcpy(ERROR_STRINGC,">>Maximum area must be greater than zero>set_flags");
      return;
    }
  }
  ...
  geomview = flag[11];
  nobound = flag[12];
  nopolywritten = flag[13];
  nonodewritten = flag[14];
  noelewritten = flag[15];
  noholes = flag[17];
  noexact = flag[18];
  ignore = flag[35]; 
...
void poolinit(pool, bytecount, itemcount, wtype, alignment, ERROR_FLAG,
  ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  if (pool->firstblock == (VOID **) NULL) {
    ...
    *ERROR_FLAG=1;
    strcpy(ERROR_STRINGC,">>Out of memory>poolinit");
    return;
  }
...
VOID *poolalloc(pool, ERROR_FLAG, ERROR_STRINGC)
struct memorypool *pool;
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
    ...
    if (pool->unallocateditems == 0) {
      ...
      if (*(pool->nowblock) == (VOID *) NULL) {
        ...
        if (newblock == (VOID **) NULL) {
          /* printf("Error:  Out of memory.\n"); */
          *ERROR_FLAG=1;
          strcpy(ERROR_STRINGC,">>Out of memory>poolalloc");
          return 0;
        }
...
void dummyinit(trianglewords, shellewords, ERROR_FLAG, ERROR_STRINGC)
int trianglewords;
int shellewords;
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  ...
  if (dummytribase == (triangle *) NULL) {
    /* printf("Error:  Out of memory.\n"); */
    *ERROR_FLAG=1;
    strcpy(ERROR_STRINGC,">>Out of memory>dummyinit");
    return;
  }
...
void initializepointpool(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  int pointsize;
  ...
  poolinit(&points, pointsize, POINTPERBLOCK,
    ...
    ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">initializepointpool");
    return;
  }
}
...
void initializetrisegpools(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  int trisize;
  ...
  if (useshelles) {
    ...
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">");
      return;
    }
  } else {
    /* Initialize the "outer space" triangle. */
    dummyinit(triangles.itemwords, 0, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">");
      return;
...
void maketriangle(newtriedge, ERROR_FLAG, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  newtriedge->tri = (triangle *) poolalloc(&triangles, ERROR_FLAG,
                                           ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">maketriangle");
    return;
  }
...
void makeshelle(newedge, ERROR_FLAG, ERROR_STRINGC)
struct edge *newedge;
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  newedge->sh = (shelle *) poolalloc(&shelles, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">makeshelle");
    return;
  }
...
point insdest;
{
  ...
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">enqueuebadtri");
    return;
  }
...
int checkedge4encroach(testedge, ERROR_FLAG, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  if (addtolist && (!nobisect || ((nobisect == 1) && (sides == 2)))) {
    ...
    badedge = (struct edge *) poolalloc(&badsegments, ERROR_FLAG,
                                        ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">checkedge4encroach");
    return 0;
  }
    if (addtolist == 1) {
...
void testtriangle(testtri, ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
  nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  if (angle > goodangle) {
    ...
    enqueuebadtri(testtri, angle, tapex, torg, tdest, ERROR_FLAG,
                  ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">testtriangle");
      return;
    }
    return;
  }
  if (vararea || fixedarea || fieldarea) {
    ...
    if (fixedarea && (area > maxarea)) {
      ...
      XD[1]=torg[1];
      XD[2]=0.0;
      area_constraint=pxj_(IBT,IDO,INP,NBJ,NEELEM,nj,
        NKJE,NPF,NPNE,
        nr,&nu,NVJE,SE,XA,XD,XE,XP);
...
void insertshelle(tri, shellemark, ERROR_FLAG, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  if (newshelle.sh == dummysh) {
    ...
    makeshelle(&newshelle, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">insertshelle");
      return;
    }
...
enum insertsiteresult insertsite(insertpoint, searchtri, splitedge,
  ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, 
  segmentflaws, triflaws, SE, XA, XE, XP, ERROR_STRINGC)
point insertpoint;
struct triedge *searchtri;
...

{
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
      ...
      if (brokenshelle.sh != dummysh) {
        ...
        if (segmentflaws) {
          if (nobisect == 0) {
            ...
            if(*ERROR_FLAG!=0)
            {
              strcat(ERROR_STRINGC,">insertsite");
    ...
    if (mirrorflag) {
      lnextself(topright);
      sym(topright, toprcasing);
      maketriangle(&newtopright, ERROR_FLAG, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">insertsite");
        return SUCCESSFULPOINT;
    ...
    maketriangle(&newbotright, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">insertsite");
      return SUCCESSFULPOINT;
    ...
    if (splitedge != (struct edge *) NULL) {
      ...
      insertshelle(&newbotright, mark(*splitedge), ERROR_FLAG, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">insertsite");
        return SUCCESSFULPOINT;
    ...
    sym(botleft, botlcasing);
    sym(botright, botrcasing);
    maketriangle(&newbotleft, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">insertsite");
      return SUCCESSFULPOINT;
    ...
    if (checksegments) {
      ...
      if (checkshelle.sh != dummysh) {
        ...
        if (segmentflaws) {
          ...
          if (checkedge4encroach(&checkshelle, ERROR_FLAG, ERROR_STRINGC)) {
            if(*ERROR_FLAG!=0)
            {
              strcat(ERROR_STRINGC,">insertsite");
              return success;
...
/*         testtriangle(&horiz, ERROR_FLAG, ERROR_STRINGC); */
        testtriangle(&horiz, ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
          nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
          if(*ERROR_FLAG!=0)
        {
          strcat(ERROR_STRINGC,">insertsite");
...
void triangulatepolygon(firstedge, lastedge, edgecount, ERROR_FLAG, doflip,
  IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, triflaws, SE,
  XA, XE, XP, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  if (bestnumber > 1) {
    ...
    triangulatepolygon(firstedge, &tempedge, bestnumber + 1, ERROR_FLAG, 1,
      IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, triflaws, SE,
      XA, XE, XP, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">triangulatepolygon");
      return;
    }
  }
  if (bestnumber < edgecount - 2) {
    ...
    triangulatepolygon(&besttri, lastedge, edgecount - bestnumber, ERROR_FLAG,
      1, IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, triflaws,
      SE, XA, XE, XP, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">triangulatepolygon");
      return;
  ...
  if (doflip) {
    ...
    if (triflaws) {
      ...
      sym(besttri, testtri);
      testtriangle(&testtri, ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
        nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">triangulatepolygon");
...
void deletesite(deltri, ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
  nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  if (edgecount > 3) {
    ...
    triangulatepolygon(&firstedge, &lastedge, edgecount, ERROR_FLAG, 0,
      IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE,
      !nobisect, SE, XA, XE, XP, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">deletesite");
      return;
    }
  }
  ...
  if (!nobisect) {
    testtriangle(deltri, ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
      nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">deletesite");
...
char *ERROR_STRINGC;
{
  ...
  maketriangle(&baseedge, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">mergehulls");
    return;
  }
  ...
  while (1) {
    ...
    if (leftfinished && rightfinished) {
      ...
      maketriangle(&nextedge, ERROR_FLAG, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
       strcat(ERROR_STRINGC,">mergehulls");
       return;
...
char *ERROR_STRINGC;
{
  ...
  if (vertices == 2) {
    ...
    maketriangle(farleft, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">divconqrecurse");
      return;
    }
    setorg(*farleft, sortarray[0]);
    setdest(*farleft, sortarray[1]);
    /* The apex is intentionally left NULL. */
    maketriangle(farright, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">divconqrecurse");
      return;
    ...
    maketriangle(&midtri, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">divconqrecurse");
      return;
    }
    maketriangle(&tri1, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">divconqrecurse");
      return;
    }
    maketriangle(&tri2, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">divconqrecurse");
      return;
    }
    maketriangle(&tri3, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">divconqrecurse");
      return;
    ...
    divconqrecurse(&sortarray[divider], vertices - divider, 1 - axis,
                   &innerright, farright, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">divconqrecurse");
      return;
...
long divconqdelaunay(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  point *sortarray;
  ...
  if (sortarray == (point *) NULL) {
    ...
    /* exit(1); */
    *ERROR_FLAG=1;
    strcpy(ERROR_STRINGC,">>Out of memory>divconqdelaunay");
    return 0;
  }
...
*/
  ...
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">divconqdelaunay");
    return 0;
  }
  free(sortarray);
...
void boundingbox(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  ...
  if ((infpoint1 == (point) NULL) || (infpoint2 == (point) NULL)
    ...
    /* exit(1); */
    *ERROR_FLAG=1;
    strcpy(ERROR_STRINGC,">>Out of memory>boundingbox");
    return;
  }
  ...
  maketriangle(&inftri, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">boundingbox");
    return;
  }
...
long incrementaldelaunay(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
  nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
int *ERROR_FLAG;
int *IBT;
int *IDO;
...
char *ERROR_STRINGC;
{
  ...
  boundingbox(ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">incrementaldelaunay");
    return 0;
  }
  if (verbose) {
...
/* insertsite(pointloop, &starttri, (struct edge *) NULL, 0, 0, */
/*                    ERROR_FLAG, ERROR_STRINGC) */
  if (insertsite(pointloop, &starttri, (struct edge *) NULL, ERROR_FLAG,
    IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, 0, 0, SE,
    XA, XE, XP, ERROR_STRINGC) == DUPLICATEPOINT) {
      if (!quiet) {
        printf(
"Warning:  A duplicate point at (%.12g, %.12g) appeared and was ignored.\n",
...
void createeventheap(eventheap, events, freeevents, ERROR_FLAG, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  *eventheap = (struct event **) malloc(maxevents * sizeof(struct event *));
  if (*eventheap == (struct event **) NULL) {
    ...
    /* exit(1); */
    *ERROR_FLAG=1;
    strcpy(ERROR_STRINGC,">>Out of memory>createeventheap");
    return;
  }
...
char *ERROR_STRINGC;
{
  ...
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">splayinsert");
    return 0;
  }
...
char *ERROR_STRINGC;
{
  ...
  return splayinsert(splay(splayroot, (point) searchpoint, &dummytri), newkey,
                     (point) searchpoint, ERROR_FLAG, ERROR_STRINGC);
}
...
long sweeplinedelaunay(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  struct event **eventheap;
  ...
  createeventheap(&eventheap, &events, &freeevents, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">sweeplinedelaunay");
    return 0;
  }
  heapsize = inpoints;
  ...
  maketriangle(&lefttri, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">sweeplinedelaunay");
    return 0;
  }
  maketriangle(&righttri, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">sweeplinedelaunay");
    return 0;
  ...
  do {
    if (heapsize == 0) {
      ...
      /* exit(1); */
      *ERROR_FLAG=1;
      strcpy(ERROR_STRINGC,">>Input points are all identical>sweeplinedelaunay");
      return 0; 
    }
...
long delaunay(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
  nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
int *ERROR_FLAG;
int *IBT;
int *IDO;
...
{
  eextras = 0;
  initializetrisegpools(ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">delaunay");
    return 0;
  }
  ...
  if (incremental) {
    return incrementaldelaunay(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
      nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
  } else if (sweepline) {
    return sweeplinedelaunay(ERROR_FLAG, ERROR_STRINGC);
  } else {
    return divconqdelaunay(ERROR_FLAG, ERROR_STRINGC);
  }
#endif /* not REDUCED */
}
...
int reconstruct(NBFM, NNM, NPNE, triangleattriblist, trianglearealist,
  elements,corners, attribs, segmentlist, segmentmarkerlist,
  numberofsegments, ERROR_FLAG, ERROR_STRINGC)
int *NBFM;
int *NNM;
int *NPNE;
...

{
  ...
  if (incorners < 3) {
    ...
    /* exit(1); */
    *ERROR_FLAG=1;
    strcpy(ERROR_STRINGC,">>Triangles must have at least 3 points>reconstruct");
    return 0;
  }
  eextras = *attribs;
#else /* not TRILIBRARY */
  ...
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">reconstruct");
    return 0;
  }

  /* Create the triangles. */
  for (elementnumber = 1; elementnumber <= inelements; elementnumber++) {
    maketriangle(&triangleloop, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">reconstruct");
      return 0;
  ...
  if (poly) {
    ...
    for (segmentnumber = 1; segmentnumber <= insegments; segmentnumber++) {
      makeshelle(&shelleloop, ERROR_FLAG, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">reconstruct");
        return 0;
  ...
  if (vertexarray == (triangle *) NULL) {
    ...
    /* exit(1); */
    *ERROR_FLAG=1;
    strcpy(ERROR_STRINGC,">>Out of memory>reconstruct");
    return 0;
  }
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < firstnumber) || (corner[j] >= firstnumber + inpoints)) {
        ...
        /* exit(1); */
        *ERROR_FLAG=1;
        strcpy(ERROR_STRINGC,">>Triangle has an invalid vertex index>reconstruct");
        return 0;
      }
    }
#else /* not TRILIBRARY */
...
#ifdef TRILIBRARY
      killpointindex = NPNE[pointindex++];
      inc2++;
#else /* not TRILIBRARY */
      stringptr = findfield(stringptr);
      if (*stringptr != '\0') {
    ...
    pointindex = pointindex + inc - inc2;
    inc2=0;
    ...
    while (shelleloop.sh != (shelle *) NULL) {
      ...
      for (j = 0; j < 2; j++) {
        if ((end[j] < firstnumber) || (end[j] >= firstnumber + inpoints)) {
          ...
          /* exit(1); */
          *ERROR_FLAG=1;
          strcpy(ERROR_STRINGC,">>Segment has an invalid vertex index>reconstruct");
          return 0;
        }
      }
          ...
          if (shorg == checkdest) {
            ...
            *prevlink = checktri.tri[6 + checktri.orient];
            ...
            if (checkneighbor.tri == dummytri) {
              ...
              insertshelle(&checktri, 1, ERROR_FLAG, ERROR_STRINGC);
              if(*ERROR_FLAG!=0)
              {
                strcat(ERROR_STRINGC,">reconstruct");
                return 0;
...
void segmentintersection(splittri, splitshelle, endpoint2, ERROR_FLAG,
  IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA,
  XE, XP, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  newpoint = (point) poolalloc(&points, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">segmentintersection");
    return;
  }
  /* Interpolate its coordinate and attributes. */
  /* Point attributes are now used to store the group numbers the node */
  /* is a member of. */
  /* Update node groups */
  ...
  for (i = 2; i < nextras + 2; i++) {
    newpoint[i] = 0.0;
    for (j = 2; j < nextras + 2; j++) {
      if(torg[i] == tdest[j]) {
        newpoint[i] = torg[i];
      }
    }
...
/*                        ERROR_FLAG, ERROR_STRINGC); */
  success = insertsite(newpoint, splittri, splitshelle, ERROR_FLAG,
    IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, 0, 0, SE,
    XA, XE, XP, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
...
int scoutsegment(searchtri, endpoint2, ERROR_FLAG, IBT, IDO, INP, NBJ,
  NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
struct triedge *searchtri;
point endpoint2;
int *ERROR_FLAG;
int *IBT;
...
char *ERROR_STRINGC;
{
  ...
  if (((leftpoint[0] == endpoint2[0]) && (leftpoint[1] == endpoint2[1])) ||
    ...
    insertshelle(searchtri, newmark, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">scoutsegment");
      return 0;
    }
    return 1;
    ...
    return scoutsegment(searchtri, endpoint2, ERROR_FLAG, IBT, IDO, INP,
      NBJ, NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, 
      ERROR_STRINGC);
    ...
    if (crossedge.sh == dummysh) {
      ...
      segmentintersection(&crosstri, &crossedge, endpoint2,
        ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE,
        nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">scoutsegment");
        return 0;
...
void conformingedge(endpoint1, endpoint2, ERROR_FLAG, IBT, IDO, INP, NBJ,
  NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP,
  ERROR_STRINGC)
point endpoint1;
point endpoint2;
int *ERROR_FLAG;
...
char *ERROR_STRINGC;
{
  ...
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">conformingedge");
    return;
  }
  ...
  for (i = 2; i < nextras + 2; i++) {
    newpoint[i] = 0.0;
    for (j = 2; j < nextras + 2; j++) {
      if(endpoint1[i] == endpoint2[j]) {
        newpoint[i] = endpoint1[i];
      }
    }
  }
...
/*                        ERROR_FLAG, ERROR_STRINGC); */
  success = insertsite(newpoint, &searchtri1, (struct edge *) NULL, ERROR_FLAG,
    IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, 0, 0, SE,
    XA, XE, XP, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
...
/*                            ERROR_FLAG, ERROR_STRINGC); */
      success = insertsite(newpoint, &searchtri1, &brokenshelle, ERROR_FLAG,
        IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, 0, 0, SE,
        XA, XE, XP, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
  ...
  result1 = scoutsegment(&searchtri1, endpoint1, ERROR_FLAG, IBT, IDO, INP,
    NBJ, NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, 
    ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
  ...
  result2 = scoutsegment(&searchtri2, endpoint2, ERROR_FLAG, IBT, IDO, INP,
    NBJ, NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, 
    ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
  ...
  if (!result1) {
    ...
    conformingedge(midpoint1, endpoint1, ERROR_FLAG, IBT, IDO, INP,
      NBJ, NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP,
      ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
  ...
  if (!result2) {
    ...
    conformingedge(midpoint2, endpoint2, ERROR_FLAG, IBT, IDO, INP,
      NBJ, NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP,
      ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
...
void constrainededge(starttri, endpoint2, ERROR_FLAG, IBT, IDO, INP, NBJ,
  NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP,
  ERROR_STRINGC)
struct triedge *starttri;
point endpoint2;
...
char *ERROR_STRINGC;
{
      ...
      if (area == 0.0) {
        ...
        if (fixupedge.sh == dummysh) {
          ...
          segmentintersection(&fixuptri, &fixupedge, endpoint2,
            ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
            nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
          if(*ERROR_FLAG!=0)
          {
            strcat(ERROR_STRINGC,">constrainededge");
            return;
          }
          done = 1;
        }
  ...
  insertshelle(&fixuptri, newmark, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">constrainededge");
    return;
  ...
  if (collision) {
    /* Insert the remainder of the segment. */
    if (!scoutsegment(&fixuptri, endpoint2, ERROR_FLAG, IBT, IDO, INP,
      NBJ, NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, 
      ERROR_STRINGC)) {
      if(*ERROR_FLAG!=0)
      {
      ...
      constrainededge(&fixuptri, endpoint2, ERROR_FLAG, IBT, IDO, INP,
        NBJ, NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, 
        ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
...
void insertsegment(endpoint1, endpoint2, ERROR_FLAG, IBT, IDO, INP, NBJ, 
  NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
point endpoint1;
point endpoint2;
int *ERROR_FLAG;
...
char *ERROR_STRINGC;
{
  ...
  /*   toward the second.                                   */
  if (scoutsegment(&searchtri1, endpoint2, ERROR_FLAG, IBT, IDO, INP,
    NBJ, NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, 
    ERROR_STRINGC)) {
  ...
  /*   toward the first.                                     */
  if (scoutsegment(&searchtri2, endpoint1, ERROR_FLAG, IBT, IDO, INP, NBJ,
    NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, 
    ERROR_STRINGC)) {
    /* The segment was easily inserted. */
    return;
  ...
  if (splitseg) {
    ...
    conformingedge(endpoint1, endpoint2, ERROR_FLAG, IBT, IDO, INP,
      NBJ,NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP,
      ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">insertsegment");
      return;
    }
  } else {
#endif /* not CDT_ONLY */
    ...
    constrainededge(&searchtri1, endpoint2, ERROR_FLAG, IBT, IDO, INP, NBJ,
      NEELEM, newmark, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, 
      ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
...
void markhull(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  ...
  do {
    ...
    insertshelle(&hulltri, 1, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">markhull");
      return;
    }
...
int formskeleton(ERROR_FLAG, IBT, IDO, INP, NBJ,NEELEM,
  nj, NKJE, NPF, NPNE, nr, numberofsegments, NVJE, segmentlist,
  segmentmarkerlist, SE, XA, XE, XP, ERROR_STRINGC)
int *ERROR_FLAG;
int *IBT;
int *IDO;
int *INP;
...

{
    ...
    for (i = 1; i <= segments; i++) {
      ...
      if ((end1 < firstnumber) || (end1 >= firstnumber + inpoints)) {
        ...
        if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
          ...
          insertsegment(endpoint1, endpoint2, ERROR_FLAG, IBT, IDO, INP,
            NBJ,NEELEM, boundmarker, nj, NKJE, NPF, NPNE, nr, NVJE, SE,
            XA, XE, XP, ERROR_STRINGC);
          if(*ERROR_FLAG!=0)
          {
            strcat(ERROR_STRINGC,">formskeleton");
            return 0;
          }
        }
      }
  ...
  if (convex || !poly) {
    ...
    markhull(ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">formskeleton");
      return 0;
...
void infecthull(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  struct triedge hulltri;
  ...
  do {
    ...
    if (!infected(hulltri)) {
      ...
      if (hulledge.sh == dummysh) {
        ...
        infect(hulltri);
        deadtri = (triangle **) poolalloc(&viri, ERROR_FLAG, ERROR_STRINGC);
        if(*ERROR_FLAG!=0)
        {
          strcat(ERROR_STRINGC,">infecthull");
          return;
        }
        *deadtri = hulltri.tri;
      } else {
...
void plague(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  struct triedge testtri;
...
void regionplague(attribute, area, ERROR_FLAG, ERROR_STRINGC)
REAL attribute;
REAL area;
int *ERROR_FLAG;
char *ERROR_STRINGC;
  ...
  while (virusloop != (triangle **) NULL) {
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if ((neighbor.tri != dummytri) && !infected(neighbor)
        ...
        infect(neighbor);
        /* Ensure that the neighbor's neighbors will be infected. */
        regiontri = (triangle **) poolalloc(&viri, ERROR_FLAG, ERROR_STRINGC);
        if(*ERROR_FLAG!=0)
        {
          strcat(ERROR_STRINGC,">regionplague");
          return;
        }
...
char *ERROR_STRINGC;
{
  ...
  if (regions > 0) {
    ...
    if (regiontris == (struct triedge *) NULL) {
      ...
      /* exit(1); */
      *ERROR_FLAG=1;
      strcpy(ERROR_STRINGC,">>Out of memory>carveholes");
      return;
    }
  }
  ...
  if (!convex) {
    ...
    infecthull(ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">carveholes");
      return;
    }
  }
  ...
  if (viri.items > 0) {
    ...
    plague(ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">carveholes");
      return;
...
void tallyencs(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  ...
  while (edgeloop.sh != (shelle *) NULL) {
    ...
    dummy = checkedge4encroach(&edgeloop, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">tallyencs");
      return;
    }
    edgeloop.sh = shelletraverse();
  }
...
void repairencs(ERROR_FLAG, flaws, IBT, IDO, INP, NBJ, NEELEM,
  nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
int *ERROR_FLAG;
int flaws;
int *IBT;
...
char *ERROR_STRINGC;
{
  ...
  while ((badsegments.items > 0) && (steinerleft != 0)) {
    ...
    while ((encloop != (struct edge *) NULL) && (steinerleft != 0)) {
      ...
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">repairencs");
        return;
       }
      /* Interpolate its coordinate and attributes. */
      /*for (i = 0; i < 2 + nextras; i++) { */
      ...
      /* printf("  arithmetic.\n"); */
        ...
        /* exit(1); */
        *ERROR_FLAG=1;
        strcpy(ERROR_STRINGC,">>split segment to a size smaller than can be accommodated by the finite precision of floating point>repairencs");
        return;
      }
      /* Insert the splitting point.  This should always succeed. */
/*       success = insertsite(newpoint, &enctri, encloop, flaws, flaws, */
/*                            ERROR_FLAG, ERROR_STRINGC); */
      success = insertsite(newpoint, &enctri, encloop, ERROR_FLAG,
        IBT, IDO, INP, NBJ,NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, flaws,
        flaws, SE, XA, XE, XP, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">repairencs");
        return;
      ...
      dummy = checkedge4encroach(encloop, ERROR_FLAG, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">repairencs");
        return;
...
void tallyfaces(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
      nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
int *ERROR_FLAG;
int *IBT;
int *IDO;
...
char *ERROR_STRINGC;
{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    testtriangle(&triangleloop, ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
      nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">tallyfaces");
      return;
    }
    triangleloop.tri = triangletraverse();
  }
...
void splittriangle(badtri, ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
  nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
...
char *ERROR_STRINGC;
{
  ...
  if ((borg == badtri->faceorg) && (bdest == badtri->facedest) &&
    ...
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">splittriangle");
      return;
    }
    ...
    if (((newpoint[0] == borg[0]) && (newpoint[1] == borg[1]))
      ...
      /* Point attributes are now used to store the group numbers the node */
      /* is a member of. */
      /* A new node within a triangle is never added to a node group */
      for (i = 2; i < nextras; i++) {
        newpoint[i] = 0.0;
      }
      if(counterclockwise(borg,newpoint,bdest)==0.0) {
        for (i = 2; i < nextras + 2; i++) {
          newpoint[i] = 0.0;
          for (j = 2; j < nextras + 2; j++) {
            if(borg[i] == bdest[j]) {
      ...
      }else if(counterclockwise(bdest,newpoint,bapex)==0.0) {
        for (i = 2; i < nextras + 2; i++) {
          newpoint[i] = 0.0;
          for (j = 2; j < nextras + 2; j++) {
            if(bdest[i] == bapex[j]) {
      ...
      }else if(counterclockwise(bapex,newpoint,borg)==0.0) {
        for (i = 2; i < nextras + 2; i++) {
          newpoint[i] = 0.0;
          for (j = 2; j < nextras + 2; j++) {
            if(bapex[i] == borg[j]) {
...
/*                            ERROR_FLAG, ERROR_STRINGC); */
      success = insertsite(newpoint, &(badtri->badfacetri),
        (struct edge *) NULL, ERROR_FLAG, IBT, IDO, INP, NBJ,NEELEM,
        nj, NKJE, NPF, NPNE, nr, NVJE, 1, 1, SE, XA, XE, XP, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">splittriangle");
        return;
      ...
      if (success == SUCCESSFULPOINT) {
        ...
        deletesite(&(badtri->badfacetri), ERROR_FLAG, IBT, IDO, INP, NBJ,
          NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP,
          ERROR_STRINGC);
        if(*ERROR_FLAG!=0)
        {
          strcat(ERROR_STRINGC,">splittriangle");
...
void enforcequality(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
  nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC)
int *ERROR_FLAG;
int *IBT;
int *IDO;
...
char *ERROR_STRINGC;
{
  ...
  tallyencs(ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">enforcequality");
    return;
  }
  ...
  while ((badsegments.items > 0) && (steinerleft != 0)) {
    ...
    repairencs(ERROR_FLAG, 0, IBT, IDO, INP, NBJ, NEELEM, nj, NKJE, NPF,
      NPNE, nr, NVJE, SE, XA, XE, XP,ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">enforcequality");
      return;
  ...
  if ((minangle > 0.0) || vararea || fixedarea) {
    ...
    tallyfaces(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
      nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">enforcequality");
    ...
    while ((badtriangles.items > 0) && (steinerleft != 0)) {
      ...
      splittriangle(dequeuebadtri(), ERROR_FLAG, IBT, IDO, INP, NBJ, 
        NEELEM, nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">enforcequality");
        return;
      ...
      if (badsegments.items > 0) {
        repairencs(ERROR_FLAG, 1, IBT, IDO, INP, NBJ, NEELEM,
          nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
        if(*ERROR_FLAG!=0)
        {
          strcat(ERROR_STRINGC,">enforcequality");
...
void highorder(ERROR_FLAG, ERROR_STRINGC)
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == dummytri)) {
        ...
        if(*ERROR_FLAG!=0)
        {
          strcat(ERROR_STRINGC,">highorder");
          return;
        }
        /*(for (i = 0; i < 2 + nextras; i++) { */ 
...
void readnodes(nodefilename, polyfilename, polyfile, ERROR_FLAG, ERROR_STRINGC)
char *nodefilename;
char *polyfilename;
FILE **polyfile;
int *ERROR_FLAG;
char *ERROR_STRINGC;
{
  ...
  initializepointpool(ERROR_FLAG, ERROR_STRINGC);

  /* Read the points. */
  for (i = 0; i < inpoints; i++) {
    pointloop = (point) poolalloc(&points, ERROR_FLAG, ERROR_STRINGC);
...
void transfernodes(NKM, NJM, NUM_NODE_VERSIONS, NVJP,
  NVM, XP, pointattriblist,pointmarkerlist, numberofnodes, numberofpoints,
  numberofpointattribs, ERROR_FLAG, ERROR_STRINGC)
int *NKM;
int *NJM;
int *NUM_NODE_VERSIONS;
int *NVJP;
...
char *ERROR_STRINGC;
{
  ...
  if (inpoints < 3) {
    ...
    /* exit(1); */
    *ERROR_FLAG=1;
    strcpy(ERROR_STRINGC,">>Input must have at least three input points>transfernodes");
    return;
  }
  ...
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">transfernodes");
    return;
  }
  ...
  for (i = 0; i < *numberofnodes; i++) {
    versionindex = coordindex;
    if (ignore) { /* Ignore the first version */
      versionindex += *NKM;
      nodeversions = NUM_NODE_VERSIONS[i] - 1;
    }
    else {
      nodeversions = NUM_NODE_VERSIONS[i];
    }
    for (k = 0; k < nodeversions; k++) { /* Loop through versions */
      pointloop = (point) poolalloc(&points, ERROR_FLAG, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">transfernodes");
      ...
      x = pointloop[0] = XP[versionindex];
      y = pointloop[1] = XP[versionindex+inc1];
      versionindex += *NKM;
      /* Read the point attributes. */
      /* for (j = 0; j < *numberofpointattribs; j++) { */
      /*   pointloop[2 + j] = pointattriblist[attribindex++]; */
/* !!! Groups in cmiss are now done differently.  This needs to be redone. */
        /* Point attributes are now used to store the group numbers the node */
        /* is a member of. */
        /* NTGRNO is the number of node groups. */
        /* node to find in list is point i + 1 */
/*        for (j = 0; j < *NTGRNO; j++) { */
/*          pointloop[2 + j] = 0.0; */
/*        } */
/*        j=0; */
/*        for(l=0;l<*NTGRNO;l++) */
/*        { */
/*          for(m=0;m<LIGRNO[l * *GRNO_MAXDATA + l];m++) */
/*          if(LIGRNO[l * *GRNO_MAXDATA + l + m + 1]==(i+1)) */
/*          { */
/*            pointloop[2 + j] = (REAL)(l+1); */
/*            j++; */
/*          } */
/*        } */
...
void writenodes(NAM, NKM, NJM, NPM, NUM_NODE_VERSIONS,
  NVJP, NVM, XP, pointattriblist, numberofnodes, pointmarkerlist,
  ERROR_FLAG, ERROR_STRINGC)
int *NAM;
int *NKM;
int *NJM;
...
/*        exit(1); */ 
/*       *ERROR_FLAG=1; */
/*       strcpy(ERROR_STRINGC,">>Out of memory>writenodes"); */
/*       return; */
/*     } */
/*   } */
  /* Check enough memory has been allocated for points*/
  if ( points.items > *NPM ) {
      *ERROR_FLAG=1;
      sprintf(ERROR_STRINGC,">>Increase NPMX to %d>writenodes", points.items );
      return;
  }
...
/*     } */
/*   } */
  /* Check enough memory has been allocated for point attributes*/
  if ( nextras > *NAM ) {
      *ERROR_FLAG=1;
      sprintf(ERROR_STRINGC,">>Increase NAMX to %d>writenodes", nextras );
      return;
  }
...
/*   plist = *pointlist; */
  ...
  /* This section of code writes out the versions to XP */
#ifdef TRILIBRARY  
  for (i = 0; i < *numberofnodes; i++) {
    versionindex = coordindex;
        versionindex = coordindex;
    ...
    for (k = 0; k < nodeversions; k++) {
      plist[versionindex] = pointloop[0];
      plist[versionindex+inc1] = pointloop[1];
      versionindex += *NKM;
      for (j = 0; j < nextras; j++) {
        palist[attribindex++] = pointloop[2 + j];
      }
      if (!nobound) {
...
/*     plist[coordindex++] = pointloop[0]; */
    ...
    plist[coordindex] = pointloop[0];
    plist[coordindex+inc1] = pointloop[1];
    coordindex=coordindex+inc2;
    /* Point attributes. */
    /*for (i = 0; i < nextras; i++) { */
...
/* !!! Groups in cmiss are now done differently.  This needs to be redone. */
      ...
      /* Update node groups */
/*      for (i = 0; i < *NTGRNO; i++) { */
/*          if((pointloop[2 + i] != 0.0) && (pointnumber > *numberofnodes)) */
/*          { */
/*            group=(int)pointloop[2 + i]-1; */
/*            num_nodes_in_group=LIGRNO[group * *GRNO_MAXDATA + group]; */
/*            LIGRNO[group * *GRNO_MAXDATA + num_nodes_in_group */
/*              + group + 1]= pointnumber; */
/*            LIGRNO[group * *GRNO_MAXDATA + group]++; */
/*          } */
/*      } */
...
#ifdef TRILIBRARY
void writeelements(NAM, NBFM, NEM, NJM, NNM, NPNE, NUM_NODE_VERSIONS, NVJE,
  numberofinitialpoints, numberofnodes,triangleattriblist, ERROR_FLAG, ERROR_STRINGC)
int *NAM;
int *NBFM;
int *NEM;
int *NJM;
...

{
  ...
  int elementnumber;
  int i;
  int inc,jumpnodebasis,jumpnodebasisdirec;
  int point1, point2, point3, pointcount;
  int FOUND, node, version, startversion;
  ...
  /* Check enough memory has been allocated for triangles*/
  if ( triangles.items  > *NEM ) {
      *ERROR_FLAG=1;
      sprintf(ERROR_STRINGC,">>Increase NEMX to %d>writeelements",
        triangles.items );
      return;
  }
  
  /* Check enough memory has been allocated for triangle attributes.*/
  if ( eextras > *NAM ) {
      *ERROR_FLAG=1;
      sprintf(ERROR_STRINGC,">>Increase NAM to %d>writeelements", eextras );
      return;
  }
  ...
  triangleloop.orient = 0;
  elementnumber = firstnumber;
  jumpnodebasis = (*NNM)*(*NBFM);
  jumpnodebasisdirec = jumpnodebasis*(*NJM);
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    if (order == 1) {
      ...
      if(point1 <= numberofinitialpoints) {
        pointcount = 0;
        FOUND = 0;
        for(node = 0; (node < *numberofnodes) && (!FOUND); node++) {
          if(ignore) {
            startversion = 1;
          }
          else {
          ...
          for(version = startversion; (version < NUM_NODE_VERSIONS[node]) && (!FOUND); version++) {
            pointcount++;
            if(pointcount == point1) FOUND = 1;
          }
        }
        tlist[jumpnodebasis*(elementnumber - 1)] = node;
        NVJE[jumpnodebasisdirec*(elementnumber - 1)] = version;
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + jumpnodebasis] = version;
      } else {
        tlist[pointindex] = point1 - (numberofinitialpoints - *numberofnodes);
        NVJE[jumpnodebasisdirec*(elementnumber - 1)] = 1;
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + jumpnodebasis] = 1;
      }
      if(point2 <= numberofinitialpoints) {
        pointcount = 0;
        FOUND = 0;
        for(node = 0; (node < *numberofnodes) && (!FOUND); node++) {
          if(ignore) {
          ...
          for(version = startversion; (version < NUM_NODE_VERSIONS[node]) && (!FOUND); version++) {
            pointcount++;
            if(pointcount == point2) FOUND = 1;
          }
        }
        tlist[jumpnodebasis*(elementnumber - 1) + 1] = node;
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + 1] = version;
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + jumpnodebasis + 1] = version;
      } else {
        tlist[pointindex+1] = point2 - (numberofinitialpoints - *numberofnodes);
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + 1] = 1;
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + jumpnodebasis + 1] = 1;
      }
      if(point3 <= numberofinitialpoints) {
        pointcount = 0;
        FOUND = 0;
        for(node = 0; (node < *numberofnodes) && (!FOUND); node++) {
          if(ignore) {
          ...
          for(version = startversion; (version < NUM_NODE_VERSIONS[node]) && (!FOUND); version++) {
            pointcount++;
            if(pointcount == point3) FOUND = 1;
          }
        }
        tlist[jumpnodebasis*(elementnumber - 1) + 2] = node;
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + 2] = version;
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + jumpnodebasis + 2] = version;
      } else {
        tlist[pointindex+2] = point3 - (numberofinitialpoints - *numberofnodes);
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + 2] = 1;
        NVJE[jumpnodebasisdirec*(elementnumber - 1) + jumpnodebasis + 2] = 1;
      }
...
#ifdef TRILIBRARY
      tlist[pointindex] = point1;
      tlist[pointindex+1] = pointmark(mid3);
      tlist[pointindex+2] = point2;
      tlist[pointindex+3] = pointmark(mid2);
      tlist[pointindex+4] = pointmark(mid1);
      tlist[pointindex+5] = point3;
      NVJE[jumpnodebasisdirec*(elementnumber - 1)] = 1;
      NVJE[jumpnodebasisdirec*(elementnumber - 1) + jumpnodebasis] = 1;
      NVJE[jumpnodebasisdirec*(elementnumber - 1) + 1] = 1;
      NVJE[jumpnodebasisdirec*(elementnumber - 1) + jumpnodebasis + 1] = 1;
...
void writepoly(NLM, segmentlist, segmentmarkerlist, ERROR_FLAG, ERROR_STRINGC)
int *NLM;
int **segmentlist;
int **segmentmarkerlist;
int *ERROR_FLAG;
...
/*        exit(1);  */
/*       *ERROR_FLAG=1; */
/*       strcpy(ERROR_STRINGC,">>Out of memory>writepoly"); */
/*       return; */
/*     } */
/*   } */
  /* Check enough memory has been allocated for segments. */
  if ( shelles.items > *NLM ) {
      *ERROR_FLAG=1;
      sprintf(ERROR_STRINGC,">>Increase NLMX to %d>writepoly", shelles.items );
      return;
  }
...
void writeedges(NLM, edgelist, edgemarkerlist, ERROR_FLAG, ERROR_STRINGC)
int *NLM;
int **edgelist;
int **edgemarkerlist;
int *ERROR_FLAG;
...
/*        exit(1);  */
/*       *ERROR_FLAG=1; */
/*       strcpy(ERROR_STRINGC,">>Out of memory>writeedges"); */
/*       return; */
/*     } */
/*   } */
  /* Check enough memory has been allocated for edges. */
  if ( edges > *NLM ) {
      *ERROR_FLAG=1;
      sprintf(ERROR_STRINGC,">>Increase NLMX to %d>writepoly", edges );
      return;
  }
...
/*                                                                           */
/*  !!!! Modified to meet requirements of fluid dynamics problems  cs 4/8/97 */
/*  !!!!The original routine is called writevoronoi_original()               */
/*                                                                           */
/*****************************************************************************/

void writevoronoi(NAM, NCM, NHM, ZA, ERROR_FLAG, ERROR_STRINGC)
int *NAM;
int *NCM;
int *NHM;
REAL *ZA;
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    plist[coordindex+inc2] = circumcenter[1];
    coordindex = coordindex + inc1;
...
void writevoronoi_original(vpointlist, vpointattriblist, vpointmarkerlist, vedgelist,
                  vedgemarkerlist, vnormlist, ERROR_FLAG, ERROR_STRINGC)
REAL **vpointlist;
...
void writevoronoi_original(vnodefilename, vedgefilename, argc, argv)
char *vnodefilename;
char *vedgefilename;
int argc;
char **argv;
...

{
  ...
  if (*vpointlist == (REAL *) NULL) {
    *vpointlist = (REAL *) malloc(triangles.items * 2 * sizeof(REAL));
    if (*vpointlist == (REAL *) NULL) {
      ...
      /* exit(1); */
      *ERROR_FLAG=1;
      strcpy(ERROR_STRINGC,">>Out of memory>writevoronoi");
      return;
    }
  }
  ...
  *vpointmarkerlist = (int *) NULL;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    /* Point attributes are now used to store the group numbers the node */
    /* is a member of. */
    /* A new node at the circumcenter is never added to a node group */
    for (i = 2; i < 2; i++) {
      /* Interpolate the point attributes at the circumcenter. */
      palist[attribindex++] = torg[i] + xi * (tdest[i] - torg[i])
                                     + eta * (tapex[i] - torg[i]);
...
void writeneighbors(NEM, NIM, NXI, ERROR_FLAG, ERROR_STRINGC)
int *NEM;
int *NIM;
int *NXI;
int *ERROR_FLAG;
...
/*        exit(1);  */
/*       *ERROR_FLAG=1; */
/*       strcpy(ERROR_STRINGC,">>Out of memory>writeneighbors"); */
/*       return; */
/*     } */
/*   } */
  /* Check enough memory has been allocated for neighbors. */
  if ( triangles.items  > *NEM ) {
      *ERROR_FLAG=1;
      sprintf(ERROR_STRINGC,">>Increase NEMX to %d>writeneighbors", triangles.items );
      return;
  }
...
#ifdef TRILIBRARY
    index = index + inc1 ;
    nlist[index+inc2] = neighbor1;
    nlist[index+inc2+inc3] = neighbor2;
    nlist[index+inc2+inc3+inc3] = neighbor3;
#else /* not TRILIBRARY */
...
/* void triangulate(triswitches, in, out, vorout, ERROR_FLAG, ERROR_STRINGC) */

void triangulate(nj, ERROR_FLAG, FLAGS, IBT, IDO, INP, NAM,
  NBFM, NBJ, NCM, NEELEM, NEM, NHM, NIM, NJM, NKJE, NKM, NLM, NNM, NPF, NPM, NPNE,
  nr,
  numberofcorners, numberofedges, numberofholes, numberofnodes, NUM_NODE_VERSIONS,
  numberofregions, numberofpoints, numberofpointattributes, numberofsegments,
  NUM_STEINER_POINTS, numberoftriangles,
  numberoftriangleattributes, NVJE, NVJP, NVM, NXI, edgelist, edgemarkerlist,           
  pointmarkerlist, segmentlist, segmentmarkerlist,  MAXIMUM_AREA, MINIMUM_ANGLE,
  trianglearealist, holelist, regionlist, pointattributelist, SE, 
  triangleattributelist, XA, XE, XP, ZA, ERROR_STRINGC )
...

{
  ...
  int numberofinitialpoints = *numberofpoints;
#ifndef TRILIBRARY
  FILE *polyfile;
#endif /* not TRILIBRARY */
#ifndef NO_TIMER
  ...
  triangleinit();
#ifdef TRILIBRARY
/*   parsecommandline(1, &triswitches, ERROR_FLAG, ERROR_STRINGC); */
  set_flags(FLAGS, NUM_STEINER_POINTS, ERROR_FLAG, MAXIMUM_AREA, MINIMUM_ANGLE, 
    ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">triangulate");
    return;
  }
...
/*                 ERROR_FLAG, ERROR_STRINGC); */
  transfernodes(NKM, NJM, NUM_NODE_VERSIONS, NVJP,
    NVM, XP, pointattributelist, pointmarkerlist, numberofnodes, numberofpoints,
    numberofpointattributes, ERROR_FLAG, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">triangulate");
    return;
...
#ifdef CDT_ONLY /* Triangulate the points. */
  hullsize = delaunay(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
    nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC);
  if(*ERROR_FLAG!=0)
  {
    strcat(ERROR_STRINGC,">triangulate");
...
#ifdef TRILIBRARY
    hullsize = reconstruct(NBFM, NNM, NPNE, triangleattributelist,
                           trianglearealist, numberoftriangles,
                           numberofcorners, numberoftriangleattributes,
...
#ifdef TRILIBRARY
      insegments = formskeleton(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
        nj, NKJE, NPF, NPNE, nr, numberofsegments, NVJE, segmentlist,
        segmentmarkerlist, SE, XA, XE, XP, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
  ...
  if (quality) {
    ...
    enforcequality(ERROR_FLAG, IBT, IDO, INP, NBJ, NEELEM,
      nj, NKJE, NPF, NPNE, nr, NVJE, SE, XA, XE, XP, ERROR_STRINGC); 
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">triangulate");
  ...
  if (order > 1) {
    highorder(ERROR_FLAG, ERROR_STRINGC); /* Promote elements to higher polynomial order. */
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">triangulate");
      return;
...
/*   out->numberofedges = edges; */
  *numberofpoints = points.items;
  *numberofpointattributes = nextras;
  *numberoftriangles = triangles.items;
  *numberofcorners = (order + 1) * (order + 2) / 2;
  *numberoftriangleattributes = eextras;
  *numberofedges = edges;
  if (useshelles) {
/*     out->numberofsegments = shelles.items; */
    *numberofsegments = shelles.items;
  } else {
/*     out->numberofsegments = hullsize; */
    *numberofsegments = hullsize;
  }
...
#ifdef TRILIBRARY
    writenodes(NAM, NKM, NJM, NPM, NUM_NODE_VERSIONS,
      NVJP, NVM, XP, &pointattributelist, numberofnodes, &pointmarkerlist,
      ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">triangulate");
...
#ifdef TRILIBRARY
    writeelements(NAM, NBFM, NEM, NJM, NNM, NPNE, NUM_NODE_VERSIONS, NVJE,
      numberofinitialpoints, numberofnodes, &triangleattributelist,
      ERROR_FLAG, ERROR_STRINGC);
     if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">triangulate");
...
#ifdef TRILIBRARY
      writepoly(NLM, &segmentlist, &segmentmarkerlist,
                ERROR_FLAG, ERROR_STRINGC);
      if(*ERROR_FLAG!=0)
      {
        strcat(ERROR_STRINGC,">triangulate");
...
/*         out->regionlist = (REAL *) NULL; */
  ...
  if (edgesout) {
#ifdef TRILIBRARY
    writeedges(NLM, &edgelist, &edgemarkerlist,
               ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">triangulate");
   ...
   if (voronoi) { 
#ifdef TRILIBRARY 
     writevoronoi(NAM, NCM, NHM, ZA, ERROR_FLAG, ERROR_STRINGC); 
     if(*ERROR_FLAG!=0) 
     { 
       strcat(ERROR_STRINGC,">triangulate"); 
       return; 
  ...
  if (neighbors) {
#ifdef TRILIBRARY
/*     writeneighbors(&out->neighborlist, ERROR_FLAG, ERROR_STRINGC); */
    writeneighbors(NEM, NIM, NXI, ERROR_FLAG, ERROR_STRINGC);
    if(*ERROR_FLAG!=0)
    {
      strcat(ERROR_STRINGC,">triangulate");
      return;
