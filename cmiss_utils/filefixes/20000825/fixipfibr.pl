...
#
# Usage:
#   ${CMISS_ROOT}/cmiss_utils/filefixes/20000825/fixipfibr.pl
#
# Created:
#   Carey Stevens 30 November 2000
#     Changes the sheet angle default convention, subtracts 90 degrees
# 
...
sub update_file
{
	...
	my $old_heading = " File Version 2";
	my $new_heading = " File Version 3";
	my $rad_or_deg_ques= " Specify whether angles entered in";
        my $rad_or_deg_line=0;
	my $rad_or_deg_ans=0;
	my $sheet_line_to_change= " The sheet angle is";
	...
	while (defined($line = <INPUT>))
	   ...
	   if($rad_or_deg_line > 0) 
           {
              $rad_or_deg_line=$rad_or_deg_line+1;
           }

           if ($line =~ m|$old_heading\E|)
           {
              $fields[6] = 3;
              foreach (@fields)
              {
           ...
           elsif ($line =~ m|$new_heading\E|)
           {
                 die "This file appears to have been converted already\nExiting without converting\n";
           }
           elsif ($line =~ m|$rad_or_deg_ques\E|)
           {
                 $rad_or_deg_line=$rad_or_deg_line+1;
                 print OUTPUT $line;
           }
           elsif ($rad_or_deg_line == 4)
           {
                 $rad_or_deg_ans=$fields[0];
                 print OUTPUT $line;
           }
           elsif ($line =~ m|^\Q$sheet_line_to_change\E|)
           {
              my $last = $#fields;
              $fields[$last] =~ s/D/E/g;
	      if($rad_or_deg_ans == 1)
	      {
                 $fields[$last] = $fields[$last]-90;
              }
	      elsif ($rad_or_deg_ans == 2)
              {
                 $fields[$last] = $fields[$last]-1.570796327;
              }
	      else 
              {
                 die "Unknown angle format\n";
              }
              foreach (@fields)
              {
                 print OUTPUT " ".$_; 
