...
sub make_changes
  {
	 ...
	 if ($line =~ m/$gfx_match_string/)
      ...
      # "create g_element_editor" -> "gfx edit scene"
 		  my $create_graphical_element_editor_match_string =
				q/(cre\w*\s+g_e\w*)(.*)/;
			if ($gfx_command =~ m/$create_graphical_element_editor_match_string/i)
			{
				my $parameters = $2;

				# remove "from ELEMENT_GROUP_NAME"
				$parameters =~ s/\s+fro\w*\s+\w+//i;

				$gfx_command =~ s/$create_graphical_element_editor_match_string/edit scene$parameters/i;
			}

      # Remove reference to default coordinate in gfx modify g_element NAME general
 		  my $modify_graphical_element_group =
				q/(mod\w*\s+g_e\w*)(.*)/;
			if ($gfx_command =~ m/$modify_graphical_element_group/i)
			{
				my $parameters = $2;

				# remove "default_coordinate default_coordinate"
				$gfx_command =~ s/\sdefa\w*\s+default_coordinate//i;
				# remove "coordinate default_coordinate"
				$gfx_command =~ s/\scoor\w*\s+default_coordinate//i;
			}

		  # gfx modify egroup
		  my $modify_egroup_match_string = q/(mod\w*\s+egr\w*\s+\w+\s+)(.+)/;
		  if ($gfx_command =~ m/$modify_egroup_match_string/i)
			{
				my $modify_command = $1;
				my $operators = $2;
				...
				$gfx_command =~ s/$modify_egroup_match_string/$modify_command$operators/;
			}
