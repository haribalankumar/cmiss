...
#
my $logfile = "$ENV{CMISS_ROOT}/filefixes/20060405/fixcom.log";
#
...
sub make_changes
  {
	 ...
	 if ($line =~ m/^\s+$gfx_match_string/)
		{
		  my $gfx_command = $1;

      # "gfx convert *"
		#  -> "gfx convert [graphics|elements] *"
 		  my $mod_texture =
				q/gfx\s+(con\w*)(.*)/;
			if ($gfx_command =~ m/$mod_texture/i)
			{
			   my $command = $1;
				...
				#If there is already "gra" or "ele" do nothing.
				if (!($parameters =~ m/^\s+(gra|ele)/))
				{
				  $gfx_command =~ s/$mod_texture/gfx convert graphics$parameters/i;
				}
			}
      ...
      # "gfx create snake "
		#  -> "gfx create snake *"
 		  my $cre_snake =
				q/gfx\s+(cre\w*\s+sna\w*)(.*)/;
			if ($gfx_command =~ m/$cre_snake/i)
			{
			  print STDERR "here\n";
			   my $command = $1;
				my $parameters = $2;

				#If there is already "fitting_fields" do nothing.
				if (!($parameters =~ m/fit/))
				{
				  #If there is a field num then expand it to the full 'number_of_elements'
				  #so it doesn't conflict with 'number_of_fitting' fields.
				  $parameters =~ s/num\w*/number_of_elements/;

				  #Find the coordinate field and specify it as a fitting_field
				  if (!($parameters =~ m/coo\s+(\w+)/))
				  {						
					 $gfx_command =~ s/$cre_snake/gfx create snake number_of_fitting_fields 1 fitting_fields $1 $parameters/i;
				  }
				}
			}
