#!/usr/bin/perl -w
#
# The is a perl script to remove all the old changes commented into command files
#
# Usage:
#   ${CMISS_ROOT}/cmiss_utils/filefixes/
#
# Created:
#   Shane Blackett 28 June 2000
#
use strict;
use File::Compare;
use File::Find;
...
#
my $LOGFILE = "/usr/people/blackett/filefixes/fixcom.log";
#
# The name of this script as written into the comfiles when changes
# are made.
#
my $SCRIPTNAME = "fixcom_commit.pl";
...
for (keys %options)
  {
    if (m%^h$%)
    ...
    elsif (m%^c$%)
      {
		  ...
		  #checked out, tested and updated.
		  $cvs_start = $options{$_}; 
      }
  }
	 ...
	 while (defined($line = <INPUT>))
		{
		  if ($line =~ m"^(> )?# Command")
			 {
				#We don't want to output the prior line;
				undef $prior_line;

				$line = <INPUT>;
				if (defined ($line) && ($line =~ m"^(> )?# Old command:"))
				  {
					 $line = <INPUT>;
					 # Get rid of the next blank line
			 ...
			 {
				#Print the prior line if it exists now
				if ($prior_line)
				  {
					 print OUTPUT $prior_line;
				  }
				if ($line =~ m/^\s$/)
				  {
					 #Keep the blank line in case we don't want it
					 $prior_line = $line;
				  }
				else
				  {
