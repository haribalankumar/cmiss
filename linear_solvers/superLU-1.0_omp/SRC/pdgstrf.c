...
#ifdef DONT_NEED_THIS_CODE
    if ( getenv("OMP_NUM_THREADS") ) {
			i = atoi(getenv("OMP_NUM_THREADS"));
			if ( nprocs > i ) {
				printf("nprocs=%d > environment allowed: OMP_NUM_THREADS=%d\n",
							 nprocs, i);
				exit(-1);
			}
    }
#endif
    if (omp_get_max_threads() != nprocs)
    {
      printf("resetting num procs %d -> %d\n", omp_get_max_threads(), nprocs);
      omp_set_num_threads(nprocs);
    }
#pragma omp parallel shared(pdgstrf_threadarg)
    {
