      ...
      subroutine BMG3_SymStd_UTILS_rV_rand(q,ii,jj,kk)
...
CC$OMP PARALLEL DO 
CC$OMP& DEFAULT(NONE)
CC$OMP& PRIVATE(i,j,k,kl)
CC$OMP& SHARED(ii,jj,kk,maxIJ,q,rZERO)
      DO kl = 0,maxIJK-1
         i = mod(kl,ii)+1
         j = mod(kl/ii,jj)+1
         k = (kl/maxIJ)+1
         if( (i.GT.2.AND.i.LT.(ii-1)).and.
     &       (j.GT.2.AND.j.LT.(jj-1)).and.
     &       (k.GT.2.AND.k.LT.(kk-1))) then
            r = 0.0D0
            q(i,j,k)= 10.0D0*RAND1(r) 
         else
            q(i,j,k) = 0.0D0
         endif
      ENDDO
...
C 
      INTEGER   ia0, ia1, ia1ma0
      PARAMETER ( ia0=1029, ia1=1536, ia1ma0=507 )
      
      INTEGER   ic
      PARAMETER ( ic=1731)

      REAL*8    rMAGIC, rONE, rP5
      PARAMETER ( rMAGIC=4194304D0, rONE=1D0, rP5=0.5D0 )
...
c
      ...
      ix1 = mod (iy1, 2048)
c
 10   rand1 = ix1*2048 + ix0
      rand1 = rand1 / rMAGIC
      return
c
 20   ix1 = mod(r,rONE)*rMAGIC + rP5
      ix0 = mod (ix1, 2048)
      ix1 = (ix1-ix0)/2048
      go to 10
