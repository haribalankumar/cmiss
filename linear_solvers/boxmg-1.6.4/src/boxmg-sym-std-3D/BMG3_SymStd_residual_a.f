...
C 
      INTEGER ifd, ii, jj, kg, kk, NOG, NStncl, BMG_IJK_MAP(*),
     &        ibmg_start
...
C$OMP& SHARED(BMG_IJK_MAP,RES,so,qf,q)
         DO KL = 0, maxIJK-1

            i = BMG_IJK_MAP(3*KL+1)
            j = BMG_IJK_MAP(3*KL+2)
            k = BMG_IJK_MAP(3*KL+3)
