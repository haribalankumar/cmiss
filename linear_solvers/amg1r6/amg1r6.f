...
C        +                  LEVELX,IFIRST,IFMG,
C        +                  NCYC,EPS,MADAPT,NRD,NSOLCO,NRU,
...
C                    IFIRST=1372815)
C
C     IFMG     -   PARAMETER FOR FMG CYCLING.
C                    =0: NO INITIAL FMG CYCLE.
C                    =1: INITIAL FMG CYCLE.
C
...
C
     ...
     +        NSTCOL(MAXGR)
      EXTERNAL SETUP1R6,FIRST1R6,SOLVE1R6,WRKCNT
      SAVE
C
      IERR = 0
C
...
C
      GOTO (10,20,30,40,50), KSWTCH
      WRITE (IUM,9920)
      IERR = 17
      RETURN
C
 40   CALL SETUP1R6 (NNU,MATRIX,KEVELX,ECG1,ECG2,EWT2,NWT,NTR,IERR,
 ...
 50   IF(KSWTCH.EQ.5) THEN
         CALL SETUP1R6 (NNU,MATRIX,KEVELX,ECG1,ECG2,EWT2,NWT,NTR,IERR,
     +            A,U,IA,JA,IG,IMIN,IMAX,IMINW,IMAXW,IG(ICGST),
     +            IG(ICGST+NDICG),NSTCOL,IARR,TIME,LEVELS,IROW0,
     +            NDA,NDIA,NDJA,NDU,NDF,NDICG,IUM,MDA,MDIA,
     +            MDJA,MDU,MDF,MDIG,MAXGR,LRATIO)
      ...
      IF (IERR.GT.0) RETURN
30    CALL FIRST1R6 (IFIRST,U,IMIN,IMAX,IARR,IROW0)
20    CALL SOLVE1R6 (MADAPT,NCYC,IFMG,NRD,NSOLCO,NRU,KOUT,IERR,A,U,F,
     +             IA,JA,IG,EPS,IMIN,IMAX,IMINW,IMAXW,IG(ICGST),
     +             IG(ICGST+NDICG),NSTCOL,IARR,TIME,NCYC0,IROW0,
     +             LEVELS,NDA,NDJA,NDU,NDF,MDA,MDJA,MDU,MDF,IUP,
     +             IUM,RESI,RES0,RES)
      IF (IERR.GT.0) RETURN
...
C.......................................................................
C
      SUBROUTINE SETUP1R6(NNU,MATRIX,LEVELX,ECG1,ECG2,EWT2,NWT,NTR,IERR,
...
C
     ...
     +        IMAXW(*),ICG(*),IFG(*),NSTCOL(*),NNU
      EXTERNAL IDEC,CHECK1R6,CRSNG
      SAVE
...
C
      IMIN(1) = 1
      IMAX(1) = NNU
      CALL CHECK1R6(IERR,A,IA,JA,IMIN,IMAX,ICG,IFG,TIME,ISYM,IROW0,NDA,
     +           NDICG,NDJA,IUM)
      IF (IERR.GT.0) RETURN
C
...
C.......................................................................
C
      SUBROUTINE CHECK1R6(IERR,A,IA,JA,IMIN,IMAX,ICG,IFG,TIME,ISYM,
     +                 IROW0,NDA,NDICG,NDJA,IUM)
C
...
C.......................................................................
C
      SUBROUTINE CHECKC(K,IERR,A,IA,JA,IW,IMIN,IMAX,IMINW,
     +             IMAXW,IFG,ICG,IUM)
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION A(*)
      INTEGER IA(*),JA(*),IMIN(*),IMAX(*)
      INTEGER IW(*),IMINW(*),IMAXW(*)
      INTEGER ICG(*),IFG(*)
      SAVE
C
...
C
      ...
      DO 20 I=ILO,IHI
        ...
        DO 10 J=JLO,JHI
          ...
          IF(R.GE.10.) ISD=1
10      CONTINUE
        IF(ISD.NE.0) NDD=NDD+1
15      CONTINUE
20    CONTINUE
      IF(NDZ.NE.0) THEN
        IERR=41
        WRITE(IUM,1000) NDZ,K
      ENDIF
      IF(NDN.NE.0) THEN
        IERR=42
        WRITE(IUM,1001) NDN,K
      ENDIF
      IF(NDD.NE.0) THEN
        IERR=43
        WRITE(IUM,1002) NDD,K
      ENDIF
      RETURN
1000  FORMAT('  ZERO DIAGONALS (',I3,') ON LEVEL ',I2)
1001  FORMAT('  NEGATIVE DIAGONALS (',I3,') ON LEVEL ',I2)
1002  FORMAT('  SMALL DIAGONALS (',I3,') ON LEVEL ',I2)
      END
C
C.......................................................................
C
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION A(*)
      DOUBLE PRECISION TOLD,TNEW,TIME(*)
      INTEGER IA(*),JA(*),IMIN(*),IMAX(*)
      EXTERNAL AMGTIME
      SAVE
C
...
C
     ...
     +        ICG(*),IFG(*),NSTCOL(*)
      EXTERNAL RWSRT,PCOL,PWINT,OPDFN,TRUNC,SETIFG,CHECKC
      SAVE
C
...
C
C     CHECK COARSE GRID MATRICES FOR POSSIBLE PROBLEMS
C
      DO 300 K=2,LEVELS
      CALL CHECKC(K,IERR,A,IA,JA,IW,IMIN,IMAX,IMINW,
     +           IMAXW,IFG,ICG,IUM)
      IF(IERR.NE.0) RETURN
300   CONTINUE
      RETURN
...
C     ******************** NEW VERSION *******************************
C
C     THIS VERSION PERFORMS THE USUAL SORT AS DESCRIBED BELOW, THEN
C     RE-SORTS THESE STRONG CONNECTIONS BASED ON SECONDARY (PATH-
C     LENGTH TWO) CONNECTIONS. THE RE-SORT IS PERFORMED FOR ALL POINTS
C     WHICH HAVE A POSITIVE OFF-DIAGONAL OR A NEIGHBOR WITH A
C     POSITIVE OFF-DIAGONAL.
C
C     ****************************************************************
C
...
C
          IF (AMX.GE.0.0D0 .OR. RS.LE.ECG1*A(IA(I))) GOTO 70
        ELSE
          DO 30 J=JLO+1,JHI
            IF (A(J).LT.AMX) THEN
              AMX = A(J)
...
C
   40   JHI = JHI-1
        IF (JLO.GE.JHI) GOTO 60
        IF (A(JHI).GT.AST) GOTO 40
C
...
C
   ...
   70 CONTINUE
C
C===>   MARK ROWS TO BE RE-SORTED (SET IA(I)=-IA(I))
C
      DO 190 I=ILO,IHI
        IAI=IABS(IA(I))
        JLO =IAI+1
...
C       APOS=-A(JLO)*ECG2
        ...
        IPOS=0
        DO 110 J=JLO,JHI
          IF(A(J).LE.APOS) GO TO 110
          IPOS=1
          IA(I)=-IAI
          IFG(JA(J))=I
  110   CONTINUE
        IF(IPOS.EQ.0) GO TO 190
        DO 140 J=JLO,JHI
          II=JA(J)
          IA(II)=-IABS(IA(II))
  140   CONTINUE
  190 CONTINUE
C
C===>   COMPUTE SECONDARY CONNECTIONS AND RE-SORT ROWS
C
      DO 200 I=ILO,IHI
        IFG(I)=0
  200 CONTINUE
C
      NRS=0
      DO 290 I=ILO,IHI
        IF(IA(I).GT.0) GO TO 290
        NRS=NRS+1
        IA(I)=-IA(I)
        JLO = IA(I)+1
        JSHI= JA(IA(I))
        IF(JLO.GT.JSHI) GO TO 290
        JHI = IABS(IA(I+1))-1
        JS0=IA(I)
        DO 210 J=JLO,JSHI
          IFG(JA(J))=J
  ...
  210   CONTINUE
        DO 230 J=JLO,JHI
          ...
          W=A(J)/A(IAII)
          JJSLO=IAII+1
          JJSHI=IABS(IA(II+1))-1
          DO 220 JJ=JJSLO,JJSHI
            III=JA(JJ)
            IF(IFG(III).GE.JS0)
     *        AA(IFG(III)-JS0)=AA(IFG(III)-JS0)-W*A(JJ)
  220     CONTINUE
  230   CONTINUE
C     WRITE(*,9110) (JA(J),A(J),J=JLO,JSHI)
C     WRITE(*,9110) (JA(J),AA(J-JS0),J=JLO,JSHI)
        JHI = JSHI
C
C===>   FIND STRONGEST CONNECTION
C
        AAMX = AA(JLO-JS0)
        AAMN = AA(JLO-JS0)
        JMX  = JLO
...
C
  260   JHI = JHI-1
        IF (JLO.GE.JHI) GOTO 280
        IF (AA(JHI-JS0).GT.AST) GOTO 260
C
C===>   INCREASE JLO UNTIL A WEAK CONNECTION IS FOUND
C       (IF JLO >= JHI STOP: ALL CONNECTIONS ARE SORTED)
C
...
C
  280   JA(IA(I)) = JHI
        JLO=IA(I)+1
C       PRINT *,'  AFTER  - I,JLO,JSHI=',I,JLO,JHI
C     WRITE(6,9111) I,JLO,JHI
9111  FORMAT('  AFTER  - I,JLO,JSHI=',3I5)
C     WRITE(*,9110) (JA(J),A(J),J=JLO,JHI)
C     WRITE(6,9110) (JA(J),AA(J-JS0),J=JLO,JHI)
9110  FORMAT(5(1X,I4,1X,D10.3))
  290 CONTINUE
C     PRINT *,'  RWSRT K,NRS=',K,NRS
C
...
C       JVALX=JVAL0+4*NTRAV+1,  NTRAV=AVERAGE NUMBER OF STRONG TRANS-
C                                     POSE CONNECTIONS (OR 1, WHICH
C                                     EVER IS GREATER)
C
...
C
     ...
     +                  NDA,NDJA,NDICG,NDIA,IUM,MDA,MDJA,IAJAS)
C
C     THIS VERSION DOES A SECONDARY DISTRIBUTION OF NON-INTERPOLATORY
C     CONNECTIONS.
C
C     NO DISTRIBUTION TO THE DIAGONAL IS USED
C
C     DISTRIBUTION (BOTH PRIMARY AND SECONDARY) IS ALONG
C     STRONG CONNECTIONS ONLY.
C
...
C     PHASE OF AMG1R4. ALSO, ICG(I) (IMIN(K)<=I<=IMAX(K)) ARE SET TO
...
C
   30   JW0 = IW(IBLCK)
        ...
        JA(JWX) = I
        IFG(I) = JWX
        DO 20 J=JLO,IA(I+1)-1
          II = JA(J)
          IF(J.GT.JHI) GO TO 10
          IF (ICG(II).LE.0) GOTO 10
          JWX = JWX+1
          IF (JWX.GT.NDAJA) GOTO 2000
          A(JWX)  = A(J)
          JA(JWX) = II
...
C
          IF(JJLO.GT.JJHI) GO TO 150
C
          DO 110 JJ=JJLO,JJHI
            IF (IFG(JA(JJ)).LT.JWD1) GOTO 110
...
C
          IF (DMAX1(S,S-SI).LE.EWT2I*A(J)*A(JJLO)) GOTO 135
C
...
C
  135   CONTINUE
          DO 140 JJ=JJLO,JJHI
            III=JA(JJ)
            IF (IFG(JA(JJ)).GE.JWD1) THEN
              AA(IFG(III)-JWS)=AA(IFG(III)-JWS)+A(JJ)
            ELSEIF(IFG(III).EQ.-I) THEN
              SS=0.
              JJJLO=IA(III)+1
              JJJHI=JA(IA(III))
              DO 142 JJJ=JJJLO,JJJHI
                IF (IFG(JA(JJJ)).GE.JWD2) SS=SS+A(JJJ)
  142         CONTINUE
              IF(SS.NE.0.0) THEN
                SN=SN+A(JJ)
                WWW = A(JJ)/SS
                DO 143 JJJ=JJJLO,JJJHI
                  IF (IFG(JA(JJJ)).GE.JWD2)
     *            AA(IFG(JA(JJJ))-JWS) = AA(IFG(JA(JJJ))-JWS)+A(JJJ)*WWW
  143           CONTINUE
              ENDIF
            ENDIF
  140     CONTINUE
          IF(S+SN.EQ.0.0) THEN
            PRINT *,' PWINT I,II,S,SN=',I,II,S,SN
            STOP 'BAD DENOMONATOR ON PWINT4'
          ENDIF
          WW=A(J)/(S+SN)
          DO 145 JJ=JW0,JWX
            A(JJ)=A(JJ)+WW*AA(JJ-JWS)
            AA(JJ-JWS)=0.
  145     CONTINUE
  150   CONTINUE
C
...
C
        DO 190 J=JHI+1,IA(I+1)-1
          ...
          DO 170 JJ=JJLO,JJHI
            ...
            IF (IFG(JA(JJ)).GE.JWD1) THEN
              S=S+A(JJ)
              AA(IFG(JA(JJ))-JWS) = AA(IFG(JA(JJ))-JWS)+A(JJ)
            ELSEIF(IFG(III).EQ.-I) THEN
              SS=0.
              JJJLO=IA(III)+1
              JJJHI=JA(IA(III))
              DO 172 JJJ=JJJLO,JJJHI
                IF (IFG(JA(JJJ)).GE.JWD2) SS=SS+A(JJJ)
  172         CONTINUE
              IF(SS.NE.0.0) THEN
                S=S+A(JJ)
                WWW = A(JJ)/SS
                DO 173 JJJ=JJJLO,JJJHI
                  IF (IFG(JA(JJJ)).GE.JWD2)
     *            AA(IFG(JA(JJJ))-JWS) = AA(IFG(JA(JJJ))-JWS)+A(JJJ)*WWW
  173           CONTINUE
              ENDIF
...
C.......................................................................
C
      SUBROUTINE SOLVE1R6(MADAPT,NCYC,IFMG,NRD,NSOLCO,NRU,IOUT,IERR,A,
     +                 U,F,IA,JA,IW,EPS,IMIN,IMAX,IMINW,IMAXW,ICG,IFG,
...
C
     ...
     +        IMINW(*),IMAXW(*),ICG(*),IFG(*),NSTCOL(*),IARR(*)
      EXTERNAL CG1R6,CYC,IDEC,RESID,USAVE
      SAVE
C
...
C      IF (RESOLD.LT.EPSIL) GOTO 170
C
      DO 120 ITER=1,NCYCLE
        CALL USAVE(1,ICGR,U,IMIN,IMAX,NDU,M,TIME,IERR,IUM,MDU,NDF,MDF)
        IF(IFMG.NE.0.AND.ITER.EQ.1) THEN
          CALL FMGCYC(NRDX,NRDTYP,NRDLEN,NRCX,NRUX,NRUTYP,NRULEN,
     ...
     +             NSC,NDA,NDJA,MDA,MDJA,MSEL,FAC,RESI,LEVELS)
        ENDIF
        CALL CG1R6(1,ICGR,ITER,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,M,TIME,
     +          IERR,IUM)
C      
C===> CALCULATE ZERO-MEAN AVERAGE
C
        IF (IROW0.EQ.1) THEN
           CALL ZMA(1,U,IMIN,IMAX)
        ENDIF
     ...
     +                                          IMIN,IMAX,IMINW)
        ...
        CFACO= RES/(RES0+1.0D-40)
        RESOLD = RES
        IF(CFACO.GT.1.D0) THEN
          IERR=50
          RETURN
        ENDIF
        IF (1.EQ.M) GOTO 150
...
C.......................................................................
C
      SUBROUTINE FMGCYC(NRDX,NRDTYP,NRDLEN,NRCX,NRUX,NRUTYP,NRULEN,
     +                  IGAM,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,IMAXW,
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION A(*),U(*),F(*),RESI(*),FAC
      DOUBLE PRECISION TIME(*)
      INTEGER NRDTYP(*),NRUTYP(*),IA(*),JA(*),IW(*),IMIN(*),IMAX(*),
     +        IMINW(*),IMAXW(*),IFG(*),ICG(*),NSTCOL(*),NG(*)
      EXTERNAL CYC,INTA,RESC,PUTZ
      SAVE
C
C     TRANSFER PROBLEM TO COARSEST GRID
C
      DO 10 K=2,M
      CALL PUTZ(K,U,IMIN,IMAX,TIME)
      CALL RESC(K,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,IMAXW,IFG,TIME)
   10 CONTINUE
C
C     PERFORM FMG PROCESS
C
      DO 20 K=M,1,-1
      CALL CYC(K,NRDX,NRDTYP,NRDLEN,NRCX,NRUX,NRUTYP,NRULEN,
     +         IGAM,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,IMAXW,
     +         IFG,ICG,NSTCOL,IARR,TIME,IROW0,M,IUM,IERR,ITER,
     +         NSC,NDA,NDJA,MDA,MDJA,MSEL,FAC,RESI,LEVELS)
      IF(K.EQ.1) GO TO 20
      CALL INTA(K-1,A,U,IA,JA,IW,IMIN,IMAX,IMINW,IMAXW,IFG,TIME)
   20 CONTINUE
      RETURN
      END
C
...
C
      ...
      DOUBLE PRECISION TIME(*),TNEW,TOLD
      INTEGER NRDTYP(*),NRUTYP(*),IA(*),JA(*),IW(*),IMIN(*),IMAX(*),
     +        IMINW(*),IMAXW(*),IFG(*),ICG(*),NSTCOL(*),NG(*)
      EXTERNAL COARSE,AMGTIME,INTA,PUTZ,RELX,RESID,VSCALE
      SAVE
C
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION A(*),U(*),F(*)
      DOUBLE PRECISION TIME(*),TOLD,TNEW
      INTEGER IA(*),JA(*),IW(*),IMIN(*),IMAX(*),IMINW(*),ICG(*),
     +        NSTCOL(*),ESP,PATH,FLAG
      EXTERNAL AMGTIME,NDRV,RELX,RESID
C
...
CC$OMP& DEFAULT(NONE)
C$OMP& PRIVATE(I,J,S)
C$OMP& SHARED(A,F,IA,ICG,JA,K,U)
...
C$OMP PARALLEL DO 
CC$OMP& DEFAULT(NONE)
C$OMP& PRIVATE(D,IC,IF,J)
C$OMP& SHARED(A,F,IA,IFG,JA,KC,U)
...
CC$OMP& DEFAULT(NONE)
C$OMP& PRIVATE(D,IC,IF,J)
C$OMP& SHARED(A,F,IA,IFG,IW,JA,KC,U)
...
C.......................................................................
C
      SUBROUTINE FIRST1R6(IFIRST,U,IMIN,IMAX,IARR,IROW0)
C
...
C$OMP PARALLEL DO REDUCTION(+:RES) 
CC$OMP& DEFAULT(NONE)
C$OMP& PRIVATE(I,J,S)
C$OMP& SHARED(A,F,IA,ICG,JA,K,U)
...
C.......................................................................
C
      SUBROUTINE CG1R6(K,ICGR,ITER,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,M,
     +              TIME,IERR,IUM)
C
...
C
      ...
      IAUX = IA(IMAX(K)+1)
      IA(IMAX(K)+1) = IW(IMINW(K))
C$OMP PARALLEL DO REDUCTION(+:S1,S2)
CC$OMP& DEFAULT(NONE)
C$OMP& PRIVATE(I,J,SR,SP)
C$OMP& SHARED(A,F,IA,ISHIFT,JA,K,U)
...
C
      ...
      IAUX = IA(IMAX(K)+1)
      IA(IMAX(K)+1) = IW(IMINW(K))
C$OMP PARALLEL DO REDUCTION(+:S1)
CC$OMP& DEFAULT(NONE)
C$OMP& PRIVATE(I,J,SR)
C$OMP& SHARED(A,F,IA,ISHIFT,JA,K,U)
      DO 50 I=IMIN(K),IMAX(K)
        SR = 0.0D0
        DO 40 J=IA(I),IA(I+1)-1
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION TIME(*),T(10),SUM1,SUM2
...
C
      ...
      DO 10 I=1,10
        T(I) = 0.0
        IF (NCYC0.GT.0) T(I) = TIME(I+10)/DBLE(NCYC0)
        SUM1 = SUM1+TIME(I)
        SUM2 = SUM2+T(I)
10    CONTINUE
C
...
C.......................................................................
C
C     ZERO_MEAN_AVERAGE                                       SUBROUTINE
C
C.......................................................................
C
      SUBROUTINE ZMA(K,U,IMIN,IMAX)
C
C     COMPUTES ZERO-MEAN AVERAGE OF U
C
      IMPLICIT NONE
      INTEGER K
      DOUBLE PRECISION U(*)
