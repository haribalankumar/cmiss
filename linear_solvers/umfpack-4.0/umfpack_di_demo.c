...
int main (void)
{
	...
	*Chain_maxrows, *Chain_maxcols, *Front_1strow, *Front_leftmostdesc,
    ...
    status = umfpack_di_symbolic (n, n, Cp, Ci, &Symbolic, Control, Info) ;
    if (status < 0)
    {
	umfpack_di_report_info (Control, Info) ;
	umfpack_di_report_status (Control, status) ;
    ...
    status = umfpack_di_get_symbolic (&nr, &nc, &anz, &nfr, &nchains,
    ...
    t = umfpack_timer ( ) - t ;
    printf ("\numfpack_di_demo complete.  Total time: %5.2f (seconds)\n", t) ;
    return (0) ;
}
