...
% Ax=b or A'x'=b', or the factorization LU=PAQ.  A must be sparse.  For the
% solve, A must be square and b must be a dense n-by-1 vector.  For LU
...
% [Ptree, Qtree, Fr, Ch, Info] = umfpack (A, Qinit, 'symbolic', Control) ;
%
...
%       Ptree is the row ordering induced by Qtree (row-merge order).
%
...
%       k is a root.  The first Fr(1,1) columns of A (Ptree,Qtree) are the pivot
%       columns for the first front, the next Fr(2,1) columns of A (Ptree,Qtree)
%       are the pivot columns for the second front, and so on.
%
%       Fr (:,3) is the row index of the first row in A (Ptree, Qtree) whose
%       leftmost nonzero entry is in a pivot column for the kth front.  Fr (:,4)
%       is the leftmost descendent of the kth front.  Rows in the range
%       Fr (Fr (k,4),3) to Fr (k+1,3)-1 form the entire set of candidate pivot
%       rows for the kth front (some of these will typically have been selected
%       as pivot rows of fronts Fr (k,3) to k-1, before the factorization
%       reaches the kth front.  If front k is a leaf node, then Fr(k,4) is k.
%
...
% If a Control entry is NaN, then the default is used for that entry.
%
