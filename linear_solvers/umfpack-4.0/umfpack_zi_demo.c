...
int main (void)
{
	...
	*Chain_maxrows, *Chain_maxcols, *Front_1strow, *Front_leftmostdesc,
    ...
    status = umfpack_zi_symbolic (n, n, Ap, Ai, &Symbolic, Control, Info) ;
    if (status < 0)
    {
    ...
    status = umfpack_zi_symbolic (n, n, Cp, Ci, &Symbolic, Control, Info) ;
    if (status < 0)
    {
	umfpack_zi_report_info (Control, Info) ;
	umfpack_zi_report_status (Control, status) ;
    ...
    status = umfpack_zi_get_symbolic (&nr, &nc, &anz, &nfr, &nchains,
    ...
    t = umfpack_timer ( ) - t ;
    printf ("\numfpack_zi_demo complete.  Total time: %5.2f (seconds)\n", t) ;
    return (0) ;
}
