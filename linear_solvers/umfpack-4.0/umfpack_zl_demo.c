...
int main (void)
{
	...
	*W, t, *Dx, rnorm ;
    double *Az, *Lz, *Uz, *Dz, *Cz ;
    long
	*Ap, *Ai, *Cp, *Ci, row, col, p, lnz, unz, nr, nc, *Lp, *Li, *Ui, *Up,
	*P, *Q, *Lj, i, j, k, anz, nfr, nchains, *Qtree, fnpiv, lnz1, unz1, nz1,
	status, *Front_npivcol, *Front_parent, *Chain_start, *Wi, *Ptree,
	*Chain_maxrows, *Chain_maxcols, *Front_1strow, *Front_leftmostdesc,
    ...
    status = umfpack_zl_symbolic (n, n, Ap, Ai, &Symbolic, Control, Info) ;
    if (status < 0)
    {
    ...
    status = umfpack_zl_symbolic (n, n, Cp, Ci, &Symbolic, Control, Info) ;
    if (status < 0)
    {
	umfpack_zl_report_info (Control, Info) ;
	umfpack_zl_report_status (Control, status) ;
    ...
    Ptree = (long *) malloc (n * sizeof (long)) ;
    Qtree = (long *) malloc (n * sizeof (long)) ;
    ...
    if (!Ptree || !Qtree || !Front_npivcol || !Front_parent || !Chain_start ||
    ...
    status = umfpack_zl_get_symbolic (&nr, &nc, &anz, &nfr, &nchains,
    ...
    Dz = (double *) NULL ;
    if (!Lp || !Lj || !Lx || !Lz || !Up || !Ui || !Ux || !Uz || !P || !Q)
    {
	error ("out of memory") ;
    }
    ...
    t = umfpack_timer ( ) - t ;
    printf ("\numfpack_zl_demo complete.  Total time: %5.2f (seconds)\n", t) ;
    return (0) ;
}
